...
- `job_runner.html` — the actual app. Single self-contained file (inline CSS + vanilla JS, no build step, no dependencies). Open it directly in a browser. On load it auto-connects to `config.js`'s `apiBase` — a connecting screen, then either an error screen (with Retry) or the login/project-picker home screen (closer to real cisTEM's own launcher), then the Assets/Actions/Results/Settings shell once a project is open. All API calls go through that same address, authenticated as the logged-in user; nothing about it is read from `localStorage` (the auth token is, the API address is not — see `config.js` below).
- `config.js` — the only file to edit to change the default API address (`window.CRYOEM_CONFIG = { apiBase: "..." }`). Loaded via a plain `<script src>` tag (not `fetch`), so it works whether `job_runner.html` is opened as `file://` or served — missing or failing to load just falls back to `http://localhost:8000/api` in `job_runner.html`'s own JS.
- `logo.png` — the cisTEM3 wordmark, shown on the connecting/error/home screens (`.home-logo`). Referenced by relative path, not embedded as a data URI, for the same reason `config.js` is a sibling file rather than inlined — keeps `job_runner.html` diffable and the asset swappable without touching the page. Not shown in the in-app topbar (too small at 46px to read well); that stays plain text.
