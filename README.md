...
| `GET`/`POST` | `/projects/:id/refinement-packages` | List packages / create one from a particle position group (`particle_group_id`) or from class selections (`selection_ids`, `recentre`, `remove_duplicates`, `duplicate_threshold_a`) — cuts the stack, writes the package and its "Random Parameters" refinement; or from an existing package (`source_package_id`, `source_refinement_id`, optional `carry_over_classes`, `class_sources`, `multi_class_rule`, `randomise_occupancies`) with that refinement's parameters carried over and the stack shared when every particle is kept |
