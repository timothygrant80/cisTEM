...
| `GET` | `/projects/:id/movies/:id/preview.png` | Summed-frame PNG preview (MRC and TIFF; EER has no preview, `415`). `?lowpass=<Å>` Gaussian low-pass filters the picture to that resolution; `?highpass=1` tapers its edges and removes density ramps |
...
| `GET` | `/projects/:id/images/:id/preview.png` | PNG preview of the micrograph (MRC and TIFF). Nothing is summed — an image is one exposure. `?lowpass=<Å>` Gaussian low-pass filters it to that resolution and `?highpass=1` tapers its edges and removes density ramps (the Find Particles panels' Low-pass and High-pass boxes) |
...
| `GET`/`POST` | `/projects/:id/refinement-packages` | List packages / create one from a particle position group (`particle_group_id`) or from class selections (`selection_ids`, `recentre`, `remove_duplicates`, `duplicate_threshold_a`) — cuts the stack, writes the package and its "Random Parameters" refinement; or from an existing package (`source_package_id`, `source_refinement_id`, optional `carry_over_classes`, `class_sources`, `multi_class_rule`, `randomise_occupancies`) with that refinement's parameters carried over and the stack shared when every particle is kept. `POST ...?async=1` answers `202 {task_id}` and runs in the background |
| `GET` | `/projects/:id/refinement-packages/tasks/:id` | A background creation's `{state, done, total, message, result?, error?}` |
| `GET` | `/projects/:id/refinement-packages/defaults` | The wizard's prefills for a group (`?particle_group_id=&largest_dimension_a=`) or for class selections (`?selection_ids=1,2` → the parent package's box size, pixel size, symmetry, molecular weight, largest dimension and class count, plus the particle count); or for an existing package (`?source_package_id=&source_refinement_id=`): its sizes and values, its refinements, and the chosen one's classes |
| `GET`/`PATCH`/`DELETE` | `/projects/:id/refinement-packages/:pid` | Details with contained particles / rename / delete (tables, and the stack file unless another package shares it) |
| `POST` | `/projects/:id/refinement-packages/:pid/export` | cisTEM's Export Refinement Package wizard: `{refinement_id, class_number, format: frealign\|relion\|relion3\|cistem, stack_path, metadata_path}` writes that refinement's parameters for one class (Frealign par, Relion 2 / 3.1 star, or cisTEM star) beside a copy of the stack, on the server's machine → `{files, particles}` |
| `POST` | `/projects/:id/refinement-packages/import` | cisTEM's Import Refinement Package wizard: `{format: cistem\|frealign\|relion, stack_path, metadata_path, name, symmetry, molecular_weight_kda, largest_dimension_a, protein_is_white, cs_mm}` (+ `pixel_size_a`, `voltage_kv`, `amplitude_contrast` for Frealign / Relion) → the new package with its "Imported Parameters" refinement (`201`) |
...
| `PATCH` | `/projects/:id/{movies,images,volumes}/:aid` | Body `{name}` renames an asset's display name (cisTEM's `RenameAsset()`; the file keeps its name) → the updated row |
...
| `POST` | `/projects/:id/jobs/:id/actions/:name` | One of the running job's `actions` (`[{name, label}]` on the `Job`): ab-initio's `take_current` / `take_last_start` stop the run and keep that reconstruction as its result; Auto Refine's and Refine 3D's `finish` stops as completed after the round in progress |
...
|---|---|---|
| `CISTEM_JOB_CONTROLLER` | `cistem_job_controller` | the default controller command; the first word must be on `PATH` or the server falls back to simulation. A run profile's own **Controller Command** overrides it for jobs on that profile |
