...
| `GET` | `/projects/:id/refinement-packages/defaults` | The wizard's prefills for a group (`?particle_group_id=&largest_dimension_a=`) or for class selections (`?selection_ids=1,2` → the parent package's box size, pixel size, symmetry, molecular weight, largest dimension and class count, plus the particle count) |
