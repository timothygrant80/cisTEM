...
namespace cufftdx {
    namespace detail {
        ...
        inline __device__ auto block_preprocess(ComplexType* input, ComplexType* smem) //
            ...
            // Move to the part of shared memory for that FFT batch, if we are doing a batched transform.
            ComplexType* smem_fft_batch;
            if constexpr (batch_size_of<FFT>::value > 1) {
                smem_fft_batch = smem + (threadIdx.y * (fft_size / 2));
            } else {
                smem_fft_batch = smem;
            }
        ...
        inline __device__ auto block_preprocess(ComplexType* input, ComplexType* smem) //
            ...
            ComplexType* smem_fft_batch;
            if constexpr (batch_size_of<FFT>::value > 1) {
                smem_fft_batch = smem + (threadIdx.y * (fft_blue_size / 2));
            } else {
                smem_fft_batch = smem;
            }
        ...
        public:
            ...
            inline __device__ void execute(T* shared_memory_input) {
                ...
                shared_to_registers<ffts_per_block>(shared_memory_input, thread_data);
                ...
                registers_to_shared<ffts_per_block>(thread_data, shared_memory_input);
            }
        ...
        private:
            ...
            inline __device__ void shared_to_registers_impl(T* shared_memory, T* thread_data, const unsigned int batch_offset) {
            ...
            template<unsigned int N, class T>
            inline __device__ void registers_to_shared_impl(T* thread_data, T* shared_memory, const unsigned int batch_offset) {
                unsigned int index = threadIdx.x;
                for(unsigned int i = 0; i < elements_per_thread; i++) {
                    if (index < N) {
            ...
            template<unsigned int FPB, class V>
            inline __device__ void shared_to_registers(void* shared_memory, V* thread_data) {
                unsigned int batch_offset;
                if constexpr (FPB > 1) {
                    batch_offset = threadIdx.y * base_type::this_fft_size_v;
                } else {
                    batch_offset = 0;
                }
                ...
                if (base_type::this_fft_type_v == fft_type::c2c) {
                    shared_to_registers_impl<base_type::this_fft_size_v>(
                        reinterpret_cast<value_type*>(shared_memory), reinterpret_cast<value_type*>(thread_data), batch_offset);
