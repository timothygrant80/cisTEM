...
namespace cufftdx {
    ...
    template<class Description>
    struct batch_size_of {
    private:
        static constexpr bool has_batch = detail::has_operator<fft_operator::ffts_per_block, Description>::value;
        static_assert(has_batch, "Description does not have batch defined");
    public:
        using value_type                  = unsigned int;
        static constexpr value_type value = detail::get_t<fft_operator::ffts_per_block, Description>::value;
        constexpr                   operator value_type() const noexcept { return value; }
    };
    
    template<class Description>
    constexpr unsigned int batch_size_of<Description>::value;
