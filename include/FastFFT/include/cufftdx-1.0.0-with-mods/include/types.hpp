...
#    if CUDA_VERSION < 11000
            __device__ __forceinline__ __host__ complex(double re, double im):
                x(__float2half2_rn(static_cast<float>(re))), y(__float2half2_rn(static_cast<float>(im))) {}
#    else
            __device__ __forceinline__ __host__ complex(double re, double im)
...
#    if CUDA_VERSION < 11000
            __device__ __forceinline__ __host__ explicit complex(const complex<double>& other):
                x(__float2half2_rn(static_cast<float>(other.real()))), y(__float2half2_rn(static_cast<float>(other.imag()))) {}
#    else
            __device__ __forceinline__ __host__ explicit complex(const complex<double>& other) {
