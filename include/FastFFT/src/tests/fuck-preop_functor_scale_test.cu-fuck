...
template <int Rank>
void preop_functor_scale(std::vector<int> size, FastFFT::SizeChangeType::Enum size_change_type, bool do_rectangle) {
    ...
    const bool set_conjMult_callback   = true;

    if ( size_change_type == SCT::decrease ) {
        std::cerr << "Decreasing the size is not yet implemented." << std::endl;
        std::exit(1);
    ...
    for ( int iSize = 0; iSize < size.size( ) - loop_limit; iSize++ ) {
        ...
        while ( oSize < loop_size ) {
            ...
            if ( (input_size.x == output_size.x && input_size.y == output_size.y && input_size.z == output_size.z) ) {
                std::cerr << "This test is only setup for block_fft_kernel_C2C_FWD_INCREASE_OP_INV_NONE" << std::endl;
                std::exit(1);
            }
            ...
            // sync before converting to fp16
            ...
            float scale_factor = positive_control.size.x * positive_control.size.y * positive_control.size.z * testVal_1 * testVal_2;
            if ( positive_control.real_values[0] == scale_factor ) {
                if ( print_out_time ) {
            ...
            FT.FwdImageInvFFT(FT_buffer, reinterpret_cast<float2*>(targetFT_buffer), FT_buffer, scale, conj_mul, noop);
...
// exit(1);
        ...
        // preop_functor_scale<2>(test_size_rectangle, do_3d, size_change_type, true);
    ...
    if ( run_3d_performance_tests ) {
        ...
        // size_change_type = SCT::increase;
        // preop_functor_scale<3>(FastFFT::test_size, do_3d, size_change_type, false);
