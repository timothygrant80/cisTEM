...
#define MyDebugAssertTrue(cond, msg, ...) if ((cond) != true) { printf(msg, ##__VA_ARGS__); printf("\nFailed Assert at %s:%i\n%s\n", __FILE__,__LINE__,__PRETTY_FUNCTION__); abort();}
#define MyDebugAssertFalse(cond, msg, ...) if ((cond) == true) { printf(msg, ##__VA_ARGS__); printf("\nFailed Assert at %s:%i\n%s\n", __FILE__,__LINE__,__PRETTY_FUNCTION__); abort();}
#else
