...
struct conditional_matrix_temp { typedef typename T::matrix_type type; };

template <typename T>
struct conditional_matrix_temp<T, true> { typedef T& type; };
...
class matrix_multiply_exp : public matrix_exp<matrix_multiply_exp<LHS, RHS>> {
  ...
  public:
    ...
    inline const type operator( )(long i) const { return matrix_exp<matrix_multiply_exp>::operator( )(i); }
    ...
    template <typename U>
    bool aliases(
            const matrix_exp<U>& item) const { return lhs.aliases(item) || rhs.aliases(item); }

    template <typename U>
    bool destructively_aliases(
            const matrix_exp<U>& item) const { return aliases(item); }
...
class matrix_add_exp : public matrix_exp<matrix_add_exp<LHS, RHS>> {
  ...
  public:
    ...
    const type operator( )(
            long r,
            long c) const { return lhs(r, c) + rhs(r, c); }

    inline const type operator( )(long i) const { return matrix_exp<matrix_add_exp>::operator( )(i); }
    ...
    template <typename U>
    bool destructively_aliases(
            const matrix_exp<U>& item) const { return lhs.destructively_aliases(item) || rhs.destructively_aliases(item); }
...
class matrix_subtract_exp : public matrix_exp<matrix_subtract_exp<LHS, RHS>> {
  ...
  public:
    ...
    inline const type operator( )(long i) const { return matrix_exp<matrix_subtract_exp>::operator( )(i); }
...
class matrix_div_scal_exp : public matrix_exp<matrix_div_scal_exp<M>> {
  ...
  public:
    ...
    const type operator( )(
            long r,
            long c) const { return m(r, c) / s; }

    inline const type operator( )(long i) const { return matrix_exp<matrix_div_scal_exp>::operator( )(i); }

    template <typename U>
    bool aliases(
            const matrix_exp<U>& item) const { return m.aliases(item); }

    template <typename U>
    bool destructively_aliases(
            const matrix_exp<U>& item) const { return m.destructively_aliases(item); }
...
template <typename T, bool is_ref>
struct conditional_reference { typedef T type; };

template <typename T>
struct conditional_reference<T, true> { typedef T& type; };
...
class matrix_mul_scal_exp : public matrix_exp<matrix_mul_scal_exp<M, use_reference>> {
  ...
  public:
    ...
    inline const type operator( )(long i) const { return matrix_exp<matrix_mul_scal_exp>::operator( )(i); }
...
inline bool operator!=(
        const matrix_exp<EXP1>& m1,
        const matrix_exp<EXP2>& m2) { return ! (m1 == m2); }
...
class matrix : public matrix_exp<matrix<T, num_rows, num_cols, mem_manager, layout>> {
  ...
  public:
    ...
    matrix(
            const matrix& m) : matrix_exp<matrix>(*this) {
        data.set_size(m.nr( ), m.nc( ));
        matrix_assign(*this, m);
    }
    ...
    template <typename U>
    bool aliases(
            const matrix_exp<U>&) const { return false; }

    bool aliases(
            const matrix_exp<matrix<T, num_rows, num_cols, mem_manager, layout>>& item) const { return (this == &item); }
...
void swap(
        ...
        matrix<T, NR, NC, mm, l>& b) { a.swap(b); }
...
class const_temp_matrix : public matrix_exp<const_temp_matrix<EXP>>, noncopyable {
  public:
    ...
    const_temp_matrix(
            const matrix_exp<EXP>& item) : ref_(item.ref( )) {}

    const_temp_matrix(
            const EXP& item) : ref_(item) {}

    const_ret_type operator( )(
            long r,
            long c) const { return ref_(r, c); }

    const_ret_type operator( )(long i) const { return ref_(i); }

    template <typename U>
    bool aliases(
            const matrix_exp<U>& item) const { return ref_.aliases(item); }

    template <typename U>
    bool destructively_aliases(
            const matrix_exp<U>& item) const { return ref_.destructively_aliases(item); }
