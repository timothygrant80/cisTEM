...
inline bool is_row_vector(
        const matrix_exp<EXP>& m) { return m.nr( ) == 1; }

template <typename EXP>
inline bool is_col_vector(
        const matrix_exp<EXP>& m) { return m.nc( ) == 1; }

template <typename EXP>
inline bool is_vector(
        const matrix_exp<EXP>& m) { return is_row_vector(m) || is_col_vector(m); }
...
struct op_binary_min : basic_op_mm<M1, M2> {
    ...
    const_ret_type apply(long r, long c) const { return std::min(this->m1(r, c), this->m2(r, c)); }
};
...
struct op_min_pointwise3 : basic_op_mmm<M1, M2, M3> {
    op_min_pointwise3(const M1& m1_, const M2& m2_, const M3& m3_) : basic_op_mmm<M1, M2, M3>(m1_, m2_, m3_) {}

    typedef typename M1::type       type;
    typedef const typename M1::type const_ret_type;
    const static long               cost = M1::cost + M2::cost + M3::cost + 2;

    const_ret_type apply(long r, long c) const { return std::min(this->m1(r, c), std::min(this->m2(r, c), this->m3(r, c))); }
};
...
struct op_binary_max : basic_op_mm<M1, M2> {
    ...
    const_ret_type apply(long r, long c) const { return std::max(this->m1(r, c), this->m2(r, c)); }
};
...
struct op_max_pointwise3 : basic_op_mmm<M1, M2, M3> {
    op_max_pointwise3(const M1& m1_, const M2& m2_, const M3& m3_) : basic_op_mmm<M1, M2, M3>(m1_, m2_, m3_) {}

    typedef typename M1::type       type;
    typedef const typename M1::type const_ret_type;
    const static long               cost = M1::cost + M2::cost + M3::cost + 2;

    const_ret_type apply(long r, long c) const { return std::max(this->m1(r, c), std::max(this->m2(r, c), this->m3(r, c))); }
};
...
struct op_cast {
    ...
    template <typename U>
    bool destructively_aliases(const matrix_exp<U>& item) const { return m.destructively_aliases(item); }
};
...
struct op_uniform_matrix_3 : does_not_alias {
    op_uniform_matrix_3(const long& rows_, const long& cols_, const T& val_) : rows(rows_), cols(cols_), val(val_) {}
...
struct op_gaussian_randm : does_not_alias {
    op_gaussian_randm(
            long          nr_,
            long          nc_,
            unsigned long seed_) : _nr(nr_), _nc(nc_), seed(seed_) {}
...
template <typename T>
struct compatible<T, T> {
    static const bool value = true;
    typedef T         type;
};

template <typename T>
struct compatible<std::complex<T>, T> {
    static const bool       value = true;
    typedef std::complex<T> type;
};

template <typename T>
struct compatible<T, std::complex<T>> {
    static const bool       value = true;
    typedef std::complex<T> type;
};
} // namespace impl
...
struct op_pointwise_multiply : basic_op_mm<M1, M2> {
    ...
    const_ret_type apply(long r, long c) const { return this->m1(r, c) * this->m2(r, c); }
};
...
struct op_pointwise_multiply3 : basic_op_mmm<M1, M2, M3> {
    op_pointwise_multiply3(const M1& m1_, const M2& m2_, const M3& m3_) : basic_op_mmm<M1, M2, M3>(m1_, m2_, m3_) {}

    typedef typename M1::type       type;
    typedef const typename M1::type const_ret_type;
    const static long               cost = M1::cost + M2::cost + M3::cost + 2;

    const_ret_type apply(long r, long c) const { return this->m1(r, c) * this->m2(r, c) * this->m3(r, c); }
};
...
struct op_pointwise_multiply4 : basic_op_mmmm<M1, M2, M3, M4> {
    op_pointwise_multiply4(const M1& m1_, const M2& m2_, const M3& m3_, const M4& m4_) : basic_op_mmmm<M1, M2, M3, M4>(m1_, m2_, m3_, m4_) {}

    typedef typename M1::type       type;
    typedef const typename M1::type const_ret_type;
    const static long               cost = M1::cost + M2::cost + M3::cost + M4::cost + 3;

    const_ret_type apply(long r, long c) const { return this->m1(r, c) * this->m2(r, c) * this->m3(r, c) * this->m4(r, c); }
};
...
struct op_pointwise_divide3 : basic_op_mmm<M1, M2, M3> {
    op_pointwise_divide3(const M1& m1_, const M2& m2_, const M3& m3_) : basic_op_mmm<M1, M2, M3>(m1_, m2_, m3_) {}
...
struct op_pointwise_divide4 : basic_op_mmmm<M1, M2, M3, M4> {
    op_pointwise_divide4(const M1& m1_, const M2& m2_, const M3& m3_, const M4& m4_) : basic_op_mmmm<M1, M2, M3, M4>(m1_, m2_, m3_, m4_) {}
...
struct op_pointwise_pow : basic_op_mm<M1, M2> {
    ...
    const_ret_type apply(long r, long c) const { return std::pow(this->m1(r, c), this->m2(r, c)); }
};
...
struct op_clamp2 : basic_op_m<M> {
    ...
    op_clamp2(const M& m_, const type& l, const type& u) : basic_op_m<M>(m_), lower(l), upper(u) {}
...
struct op_clamp_m : basic_op_mmm<M1, M2, M3> {
    op_clamp_m(const M1& m1_, const M2& m2_, const M3& m3_) : basic_op_mmm<M1, M2, M3>(m1_, m2_, m3_) {}
...
struct op_lowerbound : basic_op_m<M> {
    ...
    op_lowerbound(const M& m_, const type& thresh_) : basic_op_m<M>(m_), thresh(thresh_) {}
...
struct op_upperbound : basic_op_m<M> {
    ...
    op_upperbound(const M& m_, const type& thresh_) : basic_op_m<M>(m_), thresh(thresh_) {}
...
struct op_scale_columns {
    ...
    template <typename U>
    bool aliases(const matrix_exp<U>& item) const { return m1.aliases(item) || m2.aliases(item); }

    template <typename U>
    bool destructively_aliases(const matrix_exp<U>& item) const { return m1.destructively_aliases(item) || m2.aliases(item); }
};
...
struct op_diag_m_diag {
    ...
    template <typename U>
    bool aliases(const matrix_exp<U>& item) const { return m1.aliases(item) || m2.aliases(item) || m3.aliases(item); }

    template <typename U>
    bool destructively_aliases(const matrix_exp<U>& item) const { return m2.destructively_aliases(item) || m1.aliases(item) || m3.aliases(item); }
};
...
struct op_tensor_product {
    ...
    template <typename U>
    bool destructively_aliases(const matrix_exp<U>& item) const { return m1.aliases(item) || m2.aliases(item); }
};
...
struct op_linpiece {
    ...
    template <typename U>
    bool aliases(const matrix_exp<U>& item) const { return joints.aliases(item); }

    template <typename U>
    bool destructively_aliases(const matrix_exp<U>& item) const { return joints.aliases(item); }
};
