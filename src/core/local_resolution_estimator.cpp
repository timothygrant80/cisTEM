...
void LocalResolutionEstimator::ComputeLocalFSCAndCompareToThreshold(float fsc_threshold[], Image *local_resolution_volume, Image box_mask)
{
	...
	wxPrintf("number_of_fsc_sehlls:%i:pixel_size_in_ang:%f:\n", number_of_fsc_shells, pixel_size_in_Angstroms);
	...
	for (int shell_counter = 1; shell_counter < number_of_fsc_shells; shell_counter++)
	{
		...
		wxPrintf("current_res:%f:fsc_threshold[shell_counter]:%f:\n", current_resolution, fsc_threshold[shell_counter]);
	}

	// /// SET TIM THRESHOLD! TODO: Sort this out properly
	//consider taking out the current_resolution > x sections
	// for (int shell_counter = 1; shell_counter < number_of_fsc_shells; shell_counter++)
	// {

	// 	if (fsc_threshold[shell_counter] >= 0.95f)
	// 		fsc_threshold[shell_counter] = 0.95f;
	// 	else if (fsc_threshold[shell_counter] >= 0.93f)
	// 		fsc_threshold[shell_counter] = 0.93f;
	// 	else if (fsc_threshold[shell_counter] >= 0.9f)
	// 		fsc_threshold[shell_counter] = 0.9f;
	// 	else if (fsc_threshold[shell_counter] >= 0.75f)
	// 		fsc_threshold[shell_counter] = 0.75f;
	// 	else if (fsc_threshold[shell_counter] >= 0.5f)
	// 		fsc_threshold[shell_counter] = 0.5f;
	// 	else
	// 		fsc_threshold[shell_counter] = 0.2f;
