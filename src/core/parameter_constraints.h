...
//	~ParameterConstraints();							// destructor
	...
	inline float ReturnPhiAnglePenalty(float phi) {return - phi_noise_variance * powf(phi - phi_average,2) / 2.0 / phi_variance;};
	inline float ReturnThetaAnglePenalty(float theta) {return - theta_noise_variance * powf(theta - theta_average,2) / 2.0 / theta_variance;};
	inline float ReturnPsiAnglePenalty(float psi) {return - psi_noise_variance * powf(psi - psi_average,2) / 2.0 / psi_variance;};
	inline float ReturnShiftXPenalty(float shift_x) {return - shift_x_noise_variance * powf(shift_x - shift_x_average,2) / 2.0 / shift_x_variance;};
	inline float ReturnShiftYPenalty(float shift_y) {return - shift_y_noise_variance * powf(shift_y - shift_y_average,2) / 2.0 / shift_y_variance;};
