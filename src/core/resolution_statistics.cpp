...
// Extend table to include corners in 3D Fourier space
	...
	{
		z = pow(reconstructed_volume_1.ReturnFourierLogicalCoordGivenPhysicalCoord_Z(k) * reconstructed_volume_1.fourier_voxel_size_z, 2);

		for (j = 0; j <= reconstructed_volume_1.physical_upper_bound_complex_y; j++)
		{
			y = pow(reconstructed_volume_1.ReturnFourierLogicalCoordGivenPhysicalCoord_Y(j) * reconstructed_volume_1.fourier_voxel_size_y, 2);

			for (i = 0; i <= reconstructed_volume_1.physical_upper_bound_complex_x; i++)
			{
				x = pow(i * reconstructed_volume_1.fourier_voxel_size_x, 2);
...
//				{
					...
					temp_c = creal(reconstructed_volume_1.complex_values[pixel_counter] * conj(reconstructed_volume_2.complex_values[pixel_counter]));
					if (temp_c != 0.0)
					{
						sum1[bin] += creal(reconstructed_volume_1.complex_values[pixel_counter] * conj(reconstructed_volume_1.complex_values[pixel_counter]));
						sum2[bin] += creal(reconstructed_volume_2.complex_values[pixel_counter] * conj(reconstructed_volume_2.complex_values[pixel_counter]));
						cross_terms[bin] += creal(temp_c);
/*						if (bin == 64)
						{
							wxPrintf("%g, %g, %g\n", creal(reconstructed_volume_1.complex_values[pixel_counter] * conj(reconstructed_volume_1.complex_values[pixel_counter])),
									creal(reconstructed_volume_2.complex_values[pixel_counter] * conj(reconstructed_volume_2.complex_values[pixel_counter])), creal(temp_c));
						} */
					}
//				}
				pixel_counter++;
	...
	for (i = 0; i < number_of_bins_extended; i++)
	{
		...
		if (temp_double != 0.0)
		{
			FSC.AddPoint(pixel_size / float(i) * float(number_of_bins2), float(cross_terms[i] / sqrt(temp_double)));
		}
		else
		{
...
void ResolutionStatistics::CalculateParticleFSCandSSNR(float mask_volume_in_voxels, float molecular_mass_kDa, float pixel_size)
{
	...
	float volume_fraction = molecular_mass_kDa * kDa_to_Angstrom3 / pow(pixel_size,3) / mask_volume_in_voxels;
...
void ResolutionStatistics::CalculateParticleSSNR(Image &image_reconstruction, float *ctf_reconstruction)
{
	...
	{
		z = pow(image_reconstruction.ReturnFourierLogicalCoordGivenPhysicalCoord_Z(k) * image_reconstruction.fourier_voxel_size_z, 2);

		for (j = 0; j <= image_reconstruction.physical_upper_bound_complex_y; j++)
		{
			y = pow(image_reconstruction.ReturnFourierLogicalCoordGivenPhysicalCoord_Y(j) * image_reconstruction.fourier_voxel_size_y, 2);

			for (i = 0; i <= image_reconstruction.physical_upper_bound_complex_x; i++)
			{
				x = pow(i * image_reconstruction.fourier_voxel_size_x, 2);
				frequency_squared = x + y + z;
