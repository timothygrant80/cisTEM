 ...
 * @param n_images_to_accumulate_concurrently_ - the number of images to accumulate concurrently
 * 
 */
...
TM_EmpiricalDistribution<ccfType, mipType, per_image>::TM_EmpiricalDistribution(GpuImage&           reference_image,
                                                                                ...
                                                                                cudaStream_t        calc_stream) : n_images_to_accumulate_concurrently_{n_images_to_accumulate_concurrently_},
...
void TM_EmpiricalDistribution<ccfType, mipType, per_image>::AllocateAndZeroStatisticalArrays( ) {
    ...
    cudaErr(cudaMallocAsync(&ccf_array_.at(0), image_plane_mem_allocated_ * n_images_to_accumulate_concurrently_ * sizeof(ccfType), calc_stream_[0]));
    cudaErr(cudaMallocAsync(&ccf_array_.at(1), image_plane_mem_allocated_ * n_images_to_accumulate_concurrently_ * sizeof(ccfType), calc_stream_[0]));
    ...
    cudaErr(cudaMemsetAsync(ccf_array_.at(0), 0, image_plane_mem_allocated_ * n_images_to_accumulate_concurrently_ * sizeof(ccfType), calc_stream_[0]));
    cudaErr(cudaMemsetAsync(ccf_array_.at(1), 0, image_plane_mem_allocated_ * n_images_to_accumulate_concurrently_ * sizeof(ccfType), calc_stream_[0]));

    for ( int i = 0; i < 2; i++ ) {
        psi_array_.at(i)   = new ccfType[n_images_to_accumulate_concurrently_];
        theta_array_.at(i) = new ccfType[n_images_to_accumulate_concurrently_];
        phi_array_.at(i)   = new ccfType[n_images_to_accumulate_concurrently_];

        std::memset(psi_array_.at(i), 0, n_images_to_accumulate_concurrently_ * sizeof(ccfType));
        std::memset(theta_array_.at(i), 0, n_images_to_accumulate_concurrently_ * sizeof(ccfType));
        std::memset(phi_array_.at(i), 0, n_images_to_accumulate_concurrently_ * sizeof(ccfType));

        cudaErr(cudaMallocAsync(&d_psi_array_.at(i), n_images_to_accumulate_concurrently_ * sizeof(ccfType), calc_stream_[0]));
        cudaErr(cudaMallocAsync(&d_theta_array_.at(i), n_images_to_accumulate_concurrently_ * sizeof(ccfType), calc_stream_[0]));
        cudaErr(cudaMallocAsync(&d_phi_array_.at(i), n_images_to_accumulate_concurrently_ * sizeof(ccfType), calc_stream_[0]));

        cudaErr(cudaMemsetAsync(d_psi_array_.at(i), 0, n_images_to_accumulate_concurrently_ * sizeof(ccfType), calc_stream_[0]));
        cudaErr(cudaMemsetAsync(d_theta_array_.at(i), 0, n_images_to_accumulate_concurrently_ * sizeof(ccfType), calc_stream_[0]));
        cudaErr(cudaMemsetAsync(d_phi_array_.at(i), 0, n_images_to_accumulate_concurrently_ * sizeof(ccfType), calc_stream_[0]));
    }
...
TM_EmpiricalDistribution<ccfType, mipType, per_image>::~TM_EmpiricalDistribution( ) {
    ...
    for ( int i = 0; i < 2; i++ ) {
        delete[] psi_array_.at(i);
        delete[] theta_array_.at(i);
        delete[] phi_array_.at(i);

        cudaErr(cudaFreeAsync(d_psi_array_.at(i), calc_stream_[0]));
        cudaErr(cudaFreeAsync(d_theta_array_.at(i), calc_stream_[0]));
        cudaErr(cudaFreeAsync(d_phi_array_.at(i), calc_stream_[0]));
    }
...
template <int evalType, typename ccfType>
...
inline __device__ void write_mip_and_stats(float*    sum_array,
    ...
    if constexpr ( evalType > 0 ) {
        ...
        if constexpr ( std::is_same_v<ccfType, __half> ) {
            if ( max_val > ccfType{cistem::match_template::histogram_min} && max_val > __low2half(mip_psi[address]) ) {
                mip_psi[address]   = __halves2half2(max_val, psi[max_idx]);
                theta_phi[address] = __halves2half2(theta[max_idx], phi[max_idx]);
            }
        }
        else if constexpr ( std::is_same_v<ccfType, __nv_bfloat16> ) {
            if ( max_val > ccfType{cistem::match_template::histogram_min} && max_val > __low2bfloat16(mip_psi[address]) ) {
                mip_psi[address]   = __halves2bfloat162(max_val, psi[max_idx]);
                theta_phi[address] = __halves2bfloat162(theta[max_idx], phi[max_idx]);
            }
        }
        else if constexpr ( std::is_same_v<ccfType, histogram_storage_t> ) {
            if ( max_val > ccfType{cistem::match_template::histogram_min} && max_val > mip_psi[address].x ) {
...
__global__ void __launch_bounds__(TM::histogram_number_of_points)
    ...
    for ( int j = pre_padding.y + physical_Y( ); j < pre_padding.y + roi.y; j += GridStride_2dGrid_Y( ) ) {
        for ( int i = pre_padding.x + physical_X( ); i < pre_padding.x + roi.x; i += GridStride_2dGrid_X( ) ) {
            ...
            write_mip_and_stats<evalType>(sum_array, sum_sq_array, mip_psi, theta_phi, sum, sum_sq, psi, theta, phi, max_val, max_idx, address);
        }
    }
 ...
 * @param n_images_this_batch - number of slices to accumulate, must be <= n_images_to_accumulate_concurrently_
 */

template <typename ccfType, typename mipType, bool per_image>
void TM_EmpiricalDistribution<ccfType, mipType, per_image>::AccumulateDistribution(int n_images_this_batch) {
    ...
    if ( histogram_n_bins_ != 0 ) {
        ...
        constexpr int only_histogram = 1;
        precheck;
        AccumulateDistributionKernel<only_histogram><<<gridDims_, threadsPerBlock_, 0, calc_stream_[0]>>>(
        ...
        AccumulateDistributionKernel<histogram_and_mip><<<gridDims_, threadsPerBlock_, 0, calc_stream_[0]>>>(
                ccf_array_.at(active_idx_),
                histogram_,
                image_dims_.y,
                image_dims_.w,
