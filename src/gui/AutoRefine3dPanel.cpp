...
AutoRefine3DPanel::AutoRefine3DPanel(wxWindow *parent)
  ...
  RefinementPackageSelectPanel->AssetComboBox->Bind(
      wxEVT_COMMAND_COMBOBOX_SELECTED,
      &AutoRefine3DPanel::OnRefinementPackageComboBox, this);
  Bind(wxEVT_AUTOMASKERTHREAD_COMPLETED,
       &AutoRefine3DPanel::OnMaskerThreadComplete, this);
  Bind(wxEVT_MULTIPLY3DMASKTHREAD_COMPLETED,
       &AutoRefine3DPanel::OnMaskerThreadComplete, this);
  Bind(RETURN_PROCESSED_IMAGE_EVT, &AutoRefine3DPanel::OnOrthThreadComplete,
       this);
  Bind(wxEVT_GENERATEMASKTHREAD_COMPLETED,
       &AutoRefine3DPanel::OnGenerateMaskThreadComplete, this);
...
#include "icons/niko_picture2.cpp"
  ...
  InfoText->WriteText(wxT(
      ...
      "resolution using Frealign (Grigorieff, 2016) without the need to set "
      "many of the parameters that are required for manual refinement (see "
      "Manual Refine panel). In the simplest case, all that is required is the "
      "specification of a refinement package (set up under Assets), a starting "
      "reference (for example, a reconstruction obtained from the ab-initio "
      "procedure) and an initial resolution limit used in the refinement. The "
      "resolution should start low, for at 30 Å, to remove potential bias in "
      "the starting reference. However, for particles that are close to "
      "spherical, such as apoferritin, a higher resolution should be "
      "specified, between 8 and 12 Å (see Expert Options).  If the starting "
      "reference contains errors, the refinement may finish before converging "
      "to the correct answer.  This can often be solved by running another "
      "auto-refinement beginning with the result of the previous refinement.  "
      "In some cases multiple rounds may be needed to reach full "
      "convergence."));
  InfoText->Newline();
  InfoText->Newline();
  InfoText->EndAlignment();
  ...
  InfoText->EndBold();
  InfoText->WriteText(
      wxT("The initial 3D reconstruction used to align particles against. This "
          "should be of reasonable quality to ensure successful refinement."));
  InfoText->Newline();
  InfoText->BeginBold();
  InfoText->WriteText(wxT("Initial Res. Limit (Å) : "));
  InfoText->EndBold();
  InfoText->WriteText(wxT(
      "The starting resolution limit used to align particles against the "
      "starting reference. In most cases, this should specify a relatively low "
      "resolution to remove potential bias in the starting reference."));
  InfoText->Newline();
  InfoText->Newline();
  InfoText->EndAlignment();
  ...
  InfoText->EndBold();
  InfoText->WriteText(
      wxT("The data used for refinement is usually bandpass-limited to exclude "
          "spurious low-resolution features in the particle background (set by "
          "the low-resolution limit) and high-resolution noise (set by the "
          "high-resolution limit). It is good practice to set the "
          "low-resolution limit to 2.5x the approximate particle mask radius. "
          "The high-resolution limit should remain significantly below the "
          "resolution of the reference used for refinement to enable unbiased "
          "resolution estimation using the Fourier Shell Correlation curve."));
  InfoText->Newline();
  ...
  InfoText->EndBold();
  InfoText->WriteText(
      wxT("Radii describing a spherical mask with an inner and outer radius "
          "that will be applied to the final reconstruction and to the half "
          "reconstructions to calculate Fourier Shell Correlation curve. The "
          "inner radius is normally set to 0.0 but can assume non-zero values "
          "to remove density inside a particle if it represents largely "
          "disordered features, such as the genomic RNA or DNA of a virus."));
  InfoText->Newline();
  InfoText->Newline();
  ...
  InfoText->EndBold();
  InfoText->WriteText(wxT(
      "The radius describing the area within the boxed-out particles that "
      "contains the particles. This radius is usually larger than the particle "
      "radius to account for particles that are not perfectly centered. The "
      "best value will depend on the way the particles were picked."));
  InfoText->Newline();
  ...
  InfoText->EndBold();
  InfoText->WriteText(wxT(
      "For a global search, an angular grid search is performed and the "
      "alignment parameters for the N best matching projections are then "
      "refined further in a local refinement. Only the set of parameters "
      "yielding the best score (correlation coefficient) is kept. Increasing N "
      "will increase the chances of finding the correct particle orientations "
      "but will slow down the search. A value of 20 is recommended."));
  InfoText->Newline();
  ...
  InfoText->EndBold();
  InfoText->WriteText(wxT(
      "The global search can be limited in the X and Y directions (measured "
      "from the box center) to ensure that only particles close to the box "
      "center are found. This is useful when the particle density is high and "
      "particles end up close to each other. In this case, it is usually still "
      "possible to align all particles in a cluster of particles (assuming "
      "they do not significantly overlap). The values provided here for the "
      "search range should be set to exclude the possibility that the same "
      "particle is selected twice and counted as two different particles."));
  InfoText->Newline();
  ...
  InfoText->EndBold();
  InfoText->WriteText(
      wxT("Should the particle images be cropped to a minimum size determined "
          "by the mask radius to accelerate 3D reconstruction? This is usually "
          "not recommended as it increases interpolation artifacts."));
  InfoText->Newline();
  ...
  InfoText->EndBold();
  InfoText->WriteText(wxT(
      "Should the reconstructions be blurred by inserting each particle image "
      "at multiple orientations, weighted by a likelihood function? Enable "
      "this option if the ab-initio procedure appears to suffer from "
      "over-fitting and the appearance of spurious high-resolution features."));
  InfoText->Newline();
  ...
  InfoText->EndBold();
  InfoText->WriteText(
      wxT("A factor that reduces the range of likelihoods used for blurring. A "
          "smaller number leads to more blurring. The user should try values "
          "between 0.1 and 1."));
  InfoText->Newline();
  ...
  InfoText->EndBold();
  InfoText->WriteText(
      wxT("Should the 3D reconstructions be masked? Masking can suppress "
          "spurious density features that could be amplified during the "
          "iterative refinement. Masking should only be disabled if it appears "
          "to interfere with the reconstruction process."));
  InfoText->Newline();
  InfoText->Newline();
  ...
  InfoText->EndBold();
  InfoText->WriteText(
      wxT(" 2016. Frealign: An exploratory tool for single-particle cryo-EM. "
          "Methods Enzymol. 579, 191-226. "));
...
void AutoRefine3DPanel::ResetAllDefaultsClick(wxCommandEvent &event) {
  // TODO : should probably check that the user hasn't changed the defaults yet
  // in the future
  SetDefaults();
}
...
void AutoRefine3DPanel::SetDefaults() {
  if (RefinementPackageSelectPanel->GetCount() > 0) {
    ...
    local_mask_radius = refinement_package_asset_panel->all_refinement_packages
                            .Item(RefinementPackageSelectPanel->GetSelection())
                            .estimated_particle_size_in_angstroms *
                        0.65;
    global_mask_radius = refinement_package_asset_panel->all_refinement_packages
                             .Item(RefinementPackageSelectPanel->GetSelection())
                             .estimated_particle_size_in_angstroms *
                         0.8;

    search_range = refinement_package_asset_panel->all_refinement_packages
                       .Item(RefinementPackageSelectPanel->GetSelection())
                       .estimated_particle_size_in_angstroms *
                   0.15;
    ...
    float low_res_limit =
        refinement_package_asset_panel->all_refinement_packages
            .Item(RefinementPackageSelectPanel->GetSelection())
            .estimated_particle_size_in_angstroms *
        1.5;
    if (low_res_limit > 300.00) low_res_limit = 300.00;

    LowResolutionLimitTextCtrl->SetValue(
        wxString::Format("%.2f", low_res_limit));
    HighResolutionLimitTextCtrl->SetValue(
        wxString::Format("%.2f", calculated_high_resolution_cutoff));
    MaskRadiusTextCtrl->SetValue(wxString::Format("%.2f", local_mask_radius));

    GlobalMaskRadiusTextCtrl->SetValue(
        wxString::Format("%.2f", global_mask_radius));
...
void AutoRefine3DPanel::OnUpdateUI(wxUpdateUIEvent &event) {
  ...
  if (main_frame->current_project.is_open == false) {
    ...
    if (running_job == false) {
      ...
      if (RefinementPackageSelectPanel->GetCount() > 0 &&
          ReconstructionRunProfileComboBox->GetCount() > 0) {
        if (run_profiles_panel->run_profile_manager.ReturnTotalJobs(
                RefinementRunProfileComboBox->GetSelection()) > 0 &&
            run_profiles_panel->run_profile_manager.ReturnTotalJobs(
                ReconstructionRunProfileComboBox->GetSelection()) > 0) {
          if (RefinementPackageSelectPanel->GetSelection() != wxNOT_FOUND &&
              ReferenceSelectPanel->GetSelection() != wxNOT_FOUND) {
            if (UseMaskCheckBox->GetValue() == false ||
                MaskSelectPanel->AssetComboBox->GetSelection() != wxNOT_FOUND)
...
void AutoRefine3DPanel::OnUseMaskCheckBox(wxCommandEvent &event) {
  if (UseMaskCheckBox->GetValue() == true) {
    AutoCenterYesRadioButton->SetValue(
        false);  // should we even disable auto centering?
...
void AutoRefine3DPanel::NewRefinementPackageSelected() {
  ...
  if (RefinementPackageSelectPanel->GetCount() > 0 &&
      ReferenceSelectPanel->GetCount() > 0)
    ReferenceSelectPanel->SetSelection(
        volume_asset_panel->ReturnArrayPositionFromAssetID(
            refinement_package_asset_panel->all_refinement_packages
                .Item(RefinementPackageSelectPanel->GetSelection())
                .references_for_next_refinement[0]));
...
void AutoRefine3DPanel::OnSocketJobResultQueueMsg(
    ArrayofJobResults &received_queue) {
...
void AutoRefinementManager::SetupAndLaunchGenerateMaskThread() {
  my_parent->WriteBlueText("Setting up and launching masking threads...\n");

  int number_of_threads = 1;
  int class_counter;
  int first_slice;
  int last_slice;
  bool invalid_half_maps = false;
  ...
  float molecular_mass_kDa =
      active_refinement_package->estimated_particle_weight_in_kda;
  wxString symmetry = active_refinement_package->symmetry;

  active_refinement_run_profile =
      run_profiles_panel->run_profile_manager.run_profiles
          [my_parent->RefinementRunProfileComboBox->GetSelection()];

  int number_of_processes = active_refinement_run_profile.ReturnTotalJobs();
  my_parent->current_job_package.Reset(
      active_refinement_run_profile, "generate_local_res",
      number_of_processes * active_refinement_package->number_of_classes);

  for (class_counter = 0;
       class_counter < active_refinement_package->number_of_classes;
       class_counter++) {
    wxString half_map_1;
    wxString half_map_2;
    if (first_round_flag == true) {
      ...
      half_map_1 =
          main_frame->current_project.volume_asset_directory.GetFullPath() +
          wxString::Format("/volume_%li_%i_map1.mrc",
                           input_refinement->refinement_id, class_counter + 1);
      half_map_2 =
          main_frame->current_project.volume_asset_directory.GetFullPath() +
          wxString::Format("/volume_%li_%i_map2.mrc",
                           input_refinement->refinement_id, class_counter + 1);
    }

    if (!(halfMapExists(half_map_1) && halfMapExists(half_map_2))) {
      // my_parent->WriteBlueText(wxString::Format(wxT("Half Map 1:%s:Half Map
      // 2:%s:Highest Refinement id:%li:\n"), half_map_1, half_map_2,
      // main_frame->current_project.database.ReturnHighestRefinementID() + 1));
  ...
  if (invalid_half_maps == false) {
    for (class_counter = 0;
      ...
      // main_frame->ReturnAutoRefine3DScratchDirectory() +
      // wxString::Format("%s_%i_filtered.mrc", volume_asset_filename,
      // class_counter + 1);
      wxString half_map_1;
      wxString half_map_2;
      if (first_round_flag == true) {
      ...
      if (halfMapExists(half_map_1) && halfMapExists(half_map_2)) {
        ...
        if (active_should_mask == true) {
          ...
          mask_image_name =
              main_frame->ReturnAutoRefine3DScratchDirectory() +
              wxString::Format("%s_%i_masked.mrc", volume_asset_filename,
                               class_counter + 1);
        }
        ...
        GenerateMaskThread *mask_thread = new GenerateMaskThread(
            my_parent, half_map_1, half_map_2, mask_image_name, pixel_size,
            outer_mask_radius, active_should_mask,
            my_parent->active_mask_thread_id);

        if (mask_thread->Run() != wxTHREAD_NO_ERROR) {
          my_parent->WriteErrorText(
              "Error: Cannot start masking thread, masking will not be "
              "performed");
          delete mask_thread;
        }
      }
...
void AutoRefinementManager::SetupLocalFilteringJob() {
  ...
  num_processes_for_local_filtering = number_of_processes;
  ...
  for (class_counter = 0;
       class_counter < active_refinement_package->number_of_classes;
       class_counter++) {
    filtered_filenames.Add(main_frame->ReturnAutoRefine3DScratchDirectory() +
                           wxString::Format("%s_%i_filtered.mrc",
                                            volume_asset_filename,
                                            class_counter + 1));
  }
  ...
  for (class_counter = 0;
    ...
    if (first_round_flag == true) {
      ...
      half_map_2 =
          ...
          wxString::Format("/volume_%li_%i_map2.mrc",
                           input_refinement->refinement_id, class_counter + 1);
      input_original_volume_local_filtering =
          main_frame->current_project.volume_asset_directory.GetFullPath() +
          wxString::Format("/volume_%li_%i.mrc",
                           input_refinement->refinement_id, class_counter + 1);
    }

    float measured_resolution;
    if (first_round_flag == true) {
      measured_resolution = measured_global_resolution;
    } else {
      measured_resolution =
          input_refinement->class_refinement_results[class_counter]
              .class_resolution_statistics.ReturnEstimatedResolution();
    }
    ...
    if (halfMapExists(half_map_1) && halfMapExists(half_map_2)) {
      ...
      for (int process_counter = 0; process_counter < number_of_processes;
           process_counter++) {
        int first_slice_p = (first_slice_with_data - 1) +
                            myroundint(ReturnThreadNumberOfCurrentThread() *
                                       slices_per_process) +
                            1;
        int last_slice_p =
            (first_slice_with_data - 1) +
            myroundint((ReturnThreadNumberOfCurrentThread() + 1) *
                       slices_per_process);

        // my_parent->WriteBlueText(wxString::Format(wxT("WTW DEBUG
        // FIRST_SLICE_WITH_DATA:%i:LAST_SLICE__WITH_DATA:%i:\n"),
        // first_slice_with_data, last_slice_with_data));
        // my_parent->WriteBlueText(wxString::Format(wxT("WTW DEBUG
        // FIRST_SLICE_P:%i:LAST_SLICE_P:%i:\n"), first_slice_p, last_slice_p));

        int number_of_jobs_per_image_in_gui = 1;

        my_parent->current_job_package.AddJob(
            "ttttiiffftfifiittiti", half_map_1.ToUTF8().data(),
            half_map_2.ToUTF8().data(), output_reconstruction.ToUTF8().data(),
            mask_image_name.ToUTF8().data(), first_slice_p, last_slice_p,
            inner_mask_radius, outer_mask_radius, molecular_mass_kDa,
            symmetry.ToUTF8().data(), pixel_size, number_of_threads,
            measured_resolution,
            ...
            main_frame->current_project.volume_asset_directory.GetFullPath()
                .ToUTF8()
                .data(),
            output_reconstruction.ToUTF8().data(), class_counter + 1,
...
void AutoRefinementManager::RunLocalFilteringJob() {
  ...
  recieved_local_filtering_job_results = 0;
  ...
  current_job_id = main_frame->job_controller.AddJob(
      my_parent, active_refinement_run_profile.manager_command,
      active_refinement_run_profile.gui_address);
...
void AutoRefinementManager::BeginRefinementCycle() {
  first_round_flag =
      false;  // will get set to true if local filtering can be done

  active_refinement_package =
      &refinement_package_asset_panel->all_refinement_packages.Item(
          my_parent->RefinementPackageSelectPanel->GetSelection());
  current_refinement_package_asset_id = active_refinement_package->asset_id;

  active_low_resolution_limit =
      my_parent->LowResolutionLimitTextCtrl->ReturnValue();
  active_mask_radius = my_parent->MaskRadiusTextCtrl->ReturnValue();
  active_global_mask_radius =
      my_parent->GlobalMaskRadiusTextCtrl->ReturnValue();
  active_inner_mask_radius = my_parent->InnerMaskRadiusTextCtrl->ReturnValue();
  active_number_results_to_refine =
      my_parent->NumberToRefineSpinCtrl->GetValue();
  ...
  active_should_apply_blurring =
      my_parent->ApplyBlurringYesRadioButton->GetValue();
  ...
  if (my_parent->MaskSelectPanel->ReturnSelection() >= 0)
    active_mask_asset_id = volume_asset_panel->ReturnAssetID(
        my_parent->MaskSelectPanel->ReturnSelection());
  else
    active_mask_asset_id = -1;
  if (my_parent->MaskSelectPanel->ReturnSelection() >= 0)
    active_mask_filename = volume_asset_panel->ReturnAssetLongFilename(
        my_parent->MaskSelectPanel->ReturnSelection());
  else
    active_mask_filename = "";

  active_should_low_pass_filter_mask =
      my_parent->LowPassMaskYesRadio->GetValue();
  active_mask_filter_resolution =
      my_parent->MaskFilterResolutionText->ReturnValue();
  ...
  active_refinement_run_profile =
      run_profiles_panel->run_profile_manager.run_profiles
          [my_parent->RefinementRunProfileComboBox->GetSelection()];
  active_reconstruction_run_profile =
      run_profiles_panel->run_profile_manager.run_profiles
          [my_parent->ReconstructionRunProfileComboBox->GetSelection()];
  ...
  long number_of_particles =
      active_refinement_package->contained_particles.GetCount();
  ...
  long current_input_refinement_id =
      active_refinement_package->refinement_ids[0];

  input_refinement = main_frame->current_project.database.GetRefinementByID(
      current_input_refinement_id);
  output_refinement = new Refinement;
  output_refinement->refinement_package_asset_id =
      input_refinement->refinement_package_asset_id;
  ...
  output_refinement->SizeAndFillWithEmpty(number_of_particles,
                                          number_of_classes);
  output_refinement->refinement_id =
      main_frame->current_project.database.ReturnHighestRefinementID() + 1;
  ...
  for (class_counter = 0; class_counter < number_of_classes; class_counter++) {
    for (particle_counter = 0; particle_counter < number_of_particles;
         particle_counter++) {
      if (number_of_classes == 1)
        input_refinement->class_refinement_results[class_counter]
            .particle_refinement_results[particle_counter]
            .occupancy = 100.0;
      else
        input_refinement->class_refinement_results[class_counter]
            .particle_refinement_results[particle_counter]
            .occupancy = 100.00 / input_refinement->number_of_classes;
    ...
    input_refinement->class_refinement_results[class_counter]
        .class_resolution_statistics.GenerateDefaultStatistics(
            active_refinement_package->estimated_particle_weight_in_kda);

    class_high_res_limits.Add(
        my_parent->HighResolutionLimitTextCtrl->ReturnValue());
    class_next_high_res_limits.Add(
        my_parent->HighResolutionLimitTextCtrl->ReturnValue());
  }
  ...
  long number_of_asym_units =
      number_of_particles *
      ReturnNumberofAsymmetricUnits(active_refinement_package->symmetry);
  float estimated_required_asym_units =
      8000.0f *
      expf(75.0f /
           powf(my_parent->HighResolutionLimitTextCtrl->ReturnValue(), 2));
  long wanted_start_number_of_asym_units =
      myroundint(estimated_required_asym_units) * number_of_classes;

  // what percentage is this.

  start_percent_used =
      (float(wanted_start_number_of_asym_units) / float(number_of_asym_units)) *
      100.0;
  ...
  if (volume_asset_panel
              ->ReturnAssetPointer(
                  my_parent->ReferenceSelectPanel->ReturnSelection())
              ->x_size != active_refinement_package->stack_box_size ||
      volume_asset_panel
              ->ReturnAssetPointer(
                  my_parent->ReferenceSelectPanel->ReturnSelection())
              ->y_size != active_refinement_package->stack_box_size ||
      volume_asset_panel
              ->ReturnAssetPointer(
                  my_parent->ReferenceSelectPanel->ReturnSelection())
              ->z_size != active_refinement_package->stack_box_size ||
      fabsf(float(volume_asset_panel
                      ->ReturnAssetPointer(
                          my_parent->ReferenceSelectPanel->ReturnSelection())
                      ->pixel_size) -
            input_refinement->resolution_statistics_pixel_size) > 0.01f) {
    my_parent->WriteErrorText(wxString::Format(
        "Error: The reference volume (%i, %i, %i; psize: %f) has different "
        "dimensions / pixel size from the input stack (%i; psize: %f).  This "
  ...
  for (class_counter = 0; class_counter < number_of_classes; class_counter++) {
    current_reference_filenames.Item(class_counter) =
        volume_asset_panel->ReturnAssetLongFilename(
            my_parent->ReferenceSelectPanel->ReturnSelection());
    current_reference_asset_ids.Item(class_counter) =
        volume_asset_panel->ReturnAssetID(
            my_parent->ReferenceSelectPanel->ReturnSelection());
  }
  ...
  if (my_parent->ShowRefinementResultsPanel->LeftRightSplitter->IsSplit() ==
      true)
    my_parent->ShowRefinementResultsPanel->LeftRightSplitter->Unsplit();
  if (my_parent->ShowRefinementResultsPanel->TopBottomSplitter->IsSplit() ==
      true)
  ...
  // do not want to do local refinement if there is not a valid reconstruction
  // job id for the first round
  VolumeAsset *selected_volume = volume_asset_panel->ReturnAssetPointer(
      my_parent->ReferenceSelectPanel->ReturnSelection());
  volume_asset_filename = (selected_volume->filename).GetName();
  volume_asset_specified_half_map_1 =
      (selected_volume->half_map_1_filename).GetFullPath();
  volume_asset_specified_half_map_2 =
      (selected_volume->half_map_2_filename).GetFullPath();

  // my_parent->WriteBlueText(wxString::Format(wxT("volume_asset_filename:%s:HalfMap1:%s:HalfMap2:%s:\n"),
  // volume_asset_filename, volume_asset_specified_half_map_1,
  // volume_asset_specified_half_map_2));

  if (active_local_filtering == true &&
      selected_volume->reconstruction_job_id >= 0) {
    sqlite3_stmt *current_statement;
    main_frame->current_project.database.Prepare(
        wxString::Format("SELECT CLASS_NUMBER FROM RECONSTRUCTION_LIST WHERE "
                         "RECONSTRUCTION_ID=%li",
                         selected_volume->reconstruction_job_id),
        &current_statement);
    main_frame->current_project.database.Step(current_statement);
    int found_class = sqlite3_column_int(current_statement, 0);

    main_frame->current_project.database.Finalize(current_statement);

    long refinement_id = main_frame->current_project.database
                             .ReturnRefinementIDGivenReconstructionID(
                                 selected_volume->reconstruction_job_id);
    ShortRefinementInfo *short_info =
        refinement_package_asset_panel
            ->ReturnPointerToShortRefinementInfoByRefinementID(refinement_id);
    if (short_info != NULL) {
      if (short_info->reconstructed_volume_asset_ids[found_class - 1] ==
          selected_volume->asset_id) {
        measured_global_resolution =
            short_info->estimated_resolution[found_class - 1];
        first_round_flag = true;

        SetupAndLaunchGenerateMaskThread();
      } else {
        my_parent->WriteErrorText(
            "Volume Asset ID did not match corresponding Asset ID to class.\n");
...
void AutoRefinementManager::RunRefinementJob() {
  ...
  // expected_number_of_results = input_refinement->number_of_particles *
  // input_refinement->number_of_classes;

  output_refinement->SizeAndFillWithEmpty(input_refinement->number_of_particles,
                                          input_refinement->number_of_classes);
  // wxPrintf("Output refinement has %li particles and %i classes\n",
  // output_refinement->number_of_particles,
  // input_refinement->number_of_classes);

  current_output_refinement_id =
      main_frame->current_project.database.ReturnHighestRefinementID() + 1;

  output_refinement->refinement_id = current_output_refinement_id;

  output_refinement->name =
      wxString::Format("Auto #%li - Round %i", current_output_refinement_id,
  ...
  for (int class_counter = 0;
       class_counter < input_refinement->number_of_classes; class_counter++) {
    output_refinement->class_refinement_results[class_counter]
        .low_resolution_limit = active_low_resolution_limit;
    output_refinement->class_refinement_results[class_counter]
        .high_resolution_limit = class_high_res_limits[class_counter];
    output_refinement->class_refinement_results[class_counter].mask_radius =
        active_mask_radius;

    if (IsOdd(number_of_rounds_run) == true ||
        this_is_the_final_round == true || number_of_rounds_run == 0)
      output_refinement->class_refinement_results[class_counter]
          .signed_cc_resolution_limit = 0.0f;
    else
      output_refinement->class_refinement_results[class_counter]
          .signed_cc_resolution_limit = 15.0f;

    output_refinement->class_refinement_results[class_counter]
        .global_resolution_limit = class_high_res_limits[class_counter];
    output_refinement->class_refinement_results[class_counter]
        .global_mask_radius = active_global_mask_radius;
    output_refinement->class_refinement_results[class_counter]
        .number_results_to_refine = active_number_results_to_refine;
    output_refinement->class_refinement_results[class_counter]
        .angular_search_step = CalculateAngularStep(
        class_high_res_limits[class_counter], active_mask_radius);
    output_refinement->class_refinement_results[class_counter].search_range_x =
        active_search_range_x;
    output_refinement->class_refinement_results[class_counter].search_range_y =
        active_search_range_y;
    output_refinement->class_refinement_results[class_counter]
        .classification_resolution_limit = 10.0;
    output_refinement->class_refinement_results[class_counter]
        .should_focus_classify = false;
    output_refinement->class_refinement_results[class_counter].sphere_x_coord =
        0;
    output_refinement->class_refinement_results[class_counter].sphere_y_coord =
        0;
    output_refinement->class_refinement_results[class_counter].sphere_z_coord =
        0;
    output_refinement->class_refinement_results[class_counter]
    ...
    output_refinement->class_refinement_results[class_counter]
        .should_auto_mask = active_should_auto_mask;
    output_refinement->class_refinement_results[class_counter]
        .should_refine_input_params = true;
    output_refinement->class_refinement_results[class_counter]
        .should_use_supplied_mask = active_should_mask;
    output_refinement->class_refinement_results[class_counter].mask_asset_id =
        active_mask_asset_id;
    output_refinement->class_refinement_results[class_counter].mask_edge_width =
        active_mask_edge;
    output_refinement->class_refinement_results[class_counter]
        .outside_mask_weight = active_mask_weight;
    output_refinement->class_refinement_results[class_counter]
        .should_low_pass_filter_mask = active_should_low_pass_filter_mask;
    output_refinement->class_refinement_results[class_counter]
        .filter_resolution = active_mask_filter_resolution;
  }

  output_refinement->percent_used = current_percent_used;

  output_refinement->resolution_statistics_box_size =
      input_refinement->resolution_statistics_box_size;
  output_refinement->resolution_statistics_pixel_size =
      input_refinement->resolution_statistics_pixel_size;
  ...
  my_parent->WriteBlueText(
      wxString::Format(wxT("Running refinement round %2i (%.2f %%)\n"),
                       number_of_rounds_run + 1, current_percent_used));

  for (int class_counter = 0;
       class_counter < input_refinement->number_of_classes; class_counter++) {
    my_parent->WriteBlueText(
        wxString::Format(wxT("Res. limit for class #%i = %.2f"), class_counter,
                         class_high_res_limits[class_counter]));
  }
...
void AutoRefinementManager::SetupMerge3dJob() {
  long number_of_particles =
      active_refinement_package->contained_particles.GetCount();
  int number_of_reconstruction_jobs = std::min(
      number_of_particles, active_reconstruction_run_profile.ReturnTotalJobs());

  int class_counter;

  my_parent->current_job_package.Reset(
      active_reconstruction_run_profile, "merge3d",
      active_refinement_package->number_of_classes);
  ...
  for (class_counter = 0;
    ...
    wxString output_reconstruction_1 =
        main_frame->current_project.volume_asset_directory.GetFullPath() +
        wxString::Format("/volume_%li_%i_map1.mrc",
                         output_refinement->refinement_id, class_counter + 1);
    ...
    wxString output_reconstruction_filtered =
        main_frame->current_project.volume_asset_directory.GetFullPath() +
        wxString::Format("/volume_%li_%i.mrc", output_refinement->refinement_id,
                         class_counter + 1);
    ...
    wxString dump_file_seed_1 =
        main_frame->ReturnAutoRefine3DScratchDirectory() +
        wxString::Format("dump_file_%li_%i_odd_.dump",
                         current_output_refinement_id, class_counter);
    wxString dump_file_seed_2 =
        main_frame->ReturnAutoRefine3DScratchDirectory() +
        wxString::Format("dump_file_%li_%i_even_.dump",
                         current_output_refinement_id, class_counter);

    bool save_orthogonal_views_image = true;
    wxString orthogonal_views_filename =
        main_frame->current_project.volume_asset_directory.GetFullPath() +
        wxString::Format("/OrthViews/volume_%li_%i.mrc",
                         output_refinement->refinement_id, class_counter + 1);
    ...
    my_parent->current_job_package.AddJob(
        "ttttfffttibtiff", output_reconstruction_1.ToUTF8().data(),
        output_reconstruction_2.ToUTF8().data(),
        output_reconstruction_filtered.ToUTF8().data(),
        output_resolution_statistics.ToUTF8().data(), molecular_mass_kDa,
        inner_mask_radius, outer_mask_radius, dump_file_seed_1.ToUTF8().data(),
        dump_file_seed_2.ToUTF8().data(), class_counter + 1,
        save_orthogonal_views_image, orthogonal_views_filename.ToUTF8().data(),
...
void AutoRefinementManager::RunMerge3dJob() {
  ...
  current_job_id = main_frame->job_controller.AddJob(
      my_parent, active_reconstruction_run_profile.manager_command,
      active_reconstruction_run_profile.gui_address);
  my_parent->my_job_id = current_job_id;
...
void AutoRefinementManager::SetupReconstructionJob() {
  ...
  for (int class_counter = 0;
       class_counter < output_refinement->number_of_classes; class_counter++) {
    for (long particle_counter = 0;
         particle_counter < output_refinement->number_of_particles;
         particle_counter++) {
  ...
  written_parameter_files = output_refinement->WritecisTEMStarFiles(
      main_frame->current_project.parameter_file_directory.GetFullPath() +
          "/auto_output_par",
      1.0f, 0.0f, true);
  ...
  number_of_reconstruction_processes = std::min(
      number_of_particles, active_reconstruction_run_profile.ReturnTotalJobs());
  number_of_reconstruction_jobs = number_of_reconstruction_processes;

  my_parent->current_job_package.Reset(
      active_reconstruction_run_profile, "reconstruct3d",
  ...
  for (class_counter = 0;
       class_counter < active_refinement_package->number_of_classes;
       class_counter++) {
    for (job_counter = 0; job_counter < number_of_reconstruction_jobs;
         job_counter++) {
      FirstLastParticleForJob(first_particle, last_particle,
                              number_of_particles, job_counter + 1,
                              number_of_reconstruction_jobs);
      ...
      if (this_is_the_final_round == true)
        ...
        float padded_high_res_limit =
            input_refinement->class_refinement_results[class_counter]
                .class_resolution_statistics.ReturnResolutionNShellsAfter(
                    class_high_res_limits[class_counter],
                    output_refinement->resolution_statistics_box_size / 10);

        if (resolution_per_round.GetCount() > 0) {
          if (fabsf(resolution_per_round[resolution_per_round.GetCount() - 1] -
                    padded_high_res_limit) <
              resolution_per_round[resolution_per_round.GetCount() - 1] *
                  0.05) {
            resolution_limit_rec =
                input_refinement->class_refinement_results[class_counter]
                    .class_resolution_statistics.ReturnResolutionNShellsAfter(
                        resolution_per_round[resolution_per_round.GetCount() -
                                             1],
                        output_refinement->resolution_statistics_box_size / 10);
          } else
      ...
      if (current_percent_used * 3.0f < 100.0f)
        score_threshold =
            0.333f;  // we are refining 3 times more then current_percent_used,
                     // we want to use current percent used so it is always 1/3.
      else
        score_threshold = current_percent_used /
                          100.0;  // now 3 times current_percent_used is more
                                  // than 100%, we therefire refined them all,
                                  // and so just take current_percent used
      ...
      bool adjust_scores =
          true;  // my_parent->AdjustScoreForDefocusYesRadio->GetValue();
      ...
      wxString dump_file_1 = main_frame->ReturnAutoRefine3DScratchDirectory() +
                             wxString::Format("dump_file_%li_%i_odd_%i.dump",
                                              current_output_refinement_id,
                                              class_counter, job_counter + 1);
      wxString dump_file_2 = main_frame->ReturnAutoRefine3DScratchDirectory() +
                             wxString::Format("dump_file_%li_%i_even_%i.dump",
                                              current_output_refinement_id,
                                              class_counter, job_counter + 1);
      ...
      if (active_should_apply_blurring == true) {
        ...
        if (active_refinement_package
                ->references_for_next_refinement[class_counter] == -1) {
          input_reconstruction = "/dev/null";
          use_input_reconstruction = false;
        } else {
          input_reconstruction = current_reference_filenames.Item(
              class_counter);  // volume_asset_panel->ReturnAssetLongFilename(volume_asset_panel->ReturnArrayPositionFromAssetID(refinement_package_asset_panel->all_refinement_packages.Item(my_parent->RefinementPackageSelectPanel->GetSelection()).references_for_next_refinement[class_counter]));
      ...
      my_parent->current_job_package.AddJob(
          "ttttttttiiffffffffffbbbbbbbbbbttii",
          ...
          my_symmetry.ToUTF8().data(), first_particle, last_particle,
          output_pixel_size, molecular_mass_kDa, inner_mask_radius,
          outer_mask_radius, resolution_limit_rec, resolution_limit_ref,
          score_weight_conversion, score_threshold, smoothing_factor, padding,
          normalize_particles, adjust_scores, invert_contrast,
          exclude_blank_edges, crop_images, split_even_odd, centre_mass,
          use_input_reconstruction, threshold_input_3d, dump_arrays,
          dump_file_1.ToUTF8().data(), dump_file_2.ToUTF8().data(),
          correct_ewald_sphere, max_threads);
    }
  }
}
...
void AutoRefinementManager::RunReconstructionJob() {
  ...
  expected_number_of_results =
      output_refinement->ReturnNumberOfActiveParticlesInFirstClass() *
      output_refinement->number_of_classes;

  // in the future store the reconstruction parameters..
  // empty scratch directory..

  //	if
  //(wxDir::Exists(main_frame->current_project.scratch_directory.GetFullPath() +
  //"/Refine3D/") == true)
  //wxFileName::Rmdir(main_frame->current_project.scratch_directory.GetFullPath()
  //+ "/Refine3D/", wxPATH_RMDIR_RECURSIVE); 	if
  //(wxDir::Exists(main_frame->current_project.scratch_directory.GetFullPath() +
  //"/Refine3D/") == false)
...
void AutoRefinementManager::SetupRefinementJob() {
  ...
  for (class_counter = 0; class_counter < input_refinement->number_of_classes;
       class_counter++) {
    lowest_alignment_res =
        std::min(class_high_res_limits[class_counter], lowest_alignment_res);
  }

  // setup whether to do global or local refinement..

  for (long particle_counter = 0;
       particle_counter < input_refinement->number_of_particles;
       particle_counter++) {
    // if the measured resolution is better than 5 angstroms and the alignment
    // resolution is better than 8 and the highest resolution previously
    // globaled is greater than 9, and then do a global
    if (number_of_rounds_run == 0)
      do_global_for_this_particle = true;
    else if (resolution_per_round[resolution_per_round.GetCount() - 1] < 5.0f &&
             lowest_alignment_res <= 8.0f &&
             resolution_of_last_global_alignment[particle_counter] > 9.0f &&
             reference_3d_contains_all_particles == true &&
             number_of_rounds_run > 2)
      do_global_for_this_particle = true;
    else {
      // should we do local or global? Randonly decide..

      float round_adjust =
          powf(number_of_global_alignments[particle_counter] -
                   floor(rounds_since_global_alignment[particle_counter] / 3),
               2);
      if (round_adjust < 1) round_adjust = 1;

      // if the alignment resolution has greatly changed make it more likely to
      // global..

      float res_adjust = resolution_of_last_global_alignment[particle_counter] -
                         lowest_alignment_res;

      float likelihood_to_global;
      if (resolution_of_last_global_alignment[particle_counter] <= 5.0)
        likelihood_to_global = -5;  // don't bother doing another if we've
                                    // already done one at this resolution
      else
        likelihood_to_global =
            powf(lowest_alignment_res, 2) /
            ((1000.0f / res_adjust) * round_adjust);  // very arbritrary

      if (fabsf(global_random_number_generator.GetUniformRandom()) <
          likelihood_to_global)
        do_global_for_this_particle = true;
      else
  ...
  written_parameter_files = input_refinement->WritecisTEMStarFiles(
      main_frame->current_project.parameter_file_directory.GetFullPath() +
          "/auto_input_par",
      1.0f, 0.0f, true);
  written_res_files = input_refinement->WriteResolutionStatistics(
      main_frame->current_project.parameter_file_directory.GetFullPath() +
      "/auto_input_stats");
  ...
  number_of_refinement_processes = std::min(
      number_of_particles, active_refinement_run_profile.ReturnTotalJobs());
  number_of_refinement_jobs = number_of_refinement_processes;

  my_parent->current_job_package.Reset(
      active_refinement_run_profile, "refine3d",
      number_of_refinement_jobs * input_refinement->number_of_classes);

  for (class_counter = 0; class_counter < input_refinement->number_of_classes;
       class_counter++) {
    for (counter = 0; counter < number_of_refinement_jobs; counter++) {
      FirstLastParticleForJob(first_particle, last_particle,
                              number_of_particles, counter + 1,
                              number_of_refinement_jobs);

      wxString input_particle_images =
          active_refinement_package->stack_filename;
      wxString input_parameter_file =
          written_parameter_files.Item(class_counter);
      wxString input_reconstruction =
          current_reference_filenames.Item(class_counter);
      // my_parent->WriteBlueText(wxString::Format(wxT("input reconstruction
      // :%s:\n"), input_reconstruction)); //WTW
      wxString input_reconstruction_statistics =
          written_res_files.Item(class_counter);
      ...
      if (percent_used > 1) percent_used = 1;
        ...
        // if (percent_used > 1) percent_used = 1;
#ifdef DEBUG
      wxString output_parameter_file = wxString::Format(
          "/tmp/output_par_%li_%li.star", first_particle, last_particle);
#else
      ...
      // if (IsOdd(number_of_rounds_run) == true || this_is_the_final_round ==
      // true || number_of_rounds_run == 0) signed_CC_limit = 0.0f; else
      // signed_CC_limit = 15.0f;

      // float	 classification_resolution_limit
      // = 10.0;//class_high_res_limits[class_counter];
      // //my_parent->ClassificationHighResLimitTextCtrl->ReturnValue();
      float classification_resolution_limit =
          20.0f + (8.0f - 20.0f) * (float(number_of_rounds_run) / 9.0f);
      ...
      float angular_step = std::max(
          CalculateAngularStep(class_high_res_limits.Item(class_counter),
                               active_mask_radius),
          CalculateAngularStep(8.0, active_mask_radius));
      ...
      float mask_radius_2d =
          0;  // my_parent->SphereRadiusTextCtrl->ReturnValue();

      float defocus_search_range =
          0;  // my_parent->DefocusSearchRangeTextCtrl->ReturnValue();
      float defocus_step =
          0;  // my_parent->DefocusSearchStepTextCtrl->ReturnValue();
      float padding = 1.0;
      ...
      bool refine_x_shift =
          true;  // my_parent->RefineXShiftCheckBox->GetValue();
      bool refine_y_shift =
          true;  // my_parent->RefineYShiftCheckBox->GetValue();
      bool calculate_matching_projections = false;
      bool apply_2d_masking =
          false;  // my_parent->SphereClassificatonYesRadio->GetValue();
      ...
      my_parent->current_job_package.AddJob(
          "ttttbttttiiffffffffffffifffffffffbbbbbbbbbbbbbbbibibb",
          ...
          ouput_shift_file.ToUTF8().data(), my_symmetry.ToUTF8().data(),
          first_particle, last_particle, percent_used, output_pixel_size,
          molecular_mass_kDa, inner_mask_radius, mask_radius,
          low_resolution_limit, high_resolution_limit, signed_CC_limit,
          classification_resolution_limit, mask_radius_search,
          high_resolution_limit_search, angular_step, best_parameters_to_keep,
          max_search_x, max_search_y, mask_center_2d_x, mask_center_2d_y,
          mask_center_2d_z, mask_radius_2d, defocus_search_range, defocus_step,
          padding, global_search, local_refinement, refine_psi, refine_theta,
          refine_phi, refine_x_shift, refine_y_shift,
          calculate_matching_projections, apply_2d_masking, ctf_refinement,
          normalize_particles, invert_contrast, exclude_blank_edges,
          normalize_input_3d, threshold_input_3d, max_threads,
          global_local_refinement, class_counter, ignore_input_parameters,
          defocus_bias);
    }
  }
...
void AutoRefinementManager::ProcessJobResult(JobResult *result_to_process) {
  if (running_job_type == REFINEMENT) {
    ...
    /*		wxPrintf("Recieved a result for particle %li, x_shift = %f, y_shift =
       %f, psi = %f, theta = %f, phi = %f\n",
    ...
    MyDebugAssertTrue(current_particle != -1 && current_class != -1,
                      "Current Particle (%li) or Current Class(%i) = -1!",
                      current_particle, current_class);

    //		wxPrintf("Received a refinement result for class #%i, particle %li\n",
    //current_class + 1, current_particle + 1); wxPrintf("output refinement has
    // %i classes and %li particles\n", output_refinement->number_of_classes,
    // output_refinement->number_of_particles);

    output_refinement->class_refinement_results[current_class]
        .particle_refinement_results[current_particle]
        .position_in_stack = long(result_to_process->result_data[1] + 0.5);
    output_refinement->class_refinement_results[current_class]
        .particle_refinement_results[current_particle]
        .image_is_active = int(result_to_process->result_data[2]);
    output_refinement->class_refinement_results[current_class]
        .particle_refinement_results[current_particle]
        .psi = result_to_process->result_data[3];
    output_refinement->class_refinement_results[current_class]
        .particle_refinement_results[current_particle]
        .theta = result_to_process->result_data[4];
    output_refinement->class_refinement_results[current_class]
        .particle_refinement_results[current_particle]
        .phi = result_to_process->result_data[5];
    output_refinement->class_refinement_results[current_class]
        .particle_refinement_results[current_particle]
        .xshift = result_to_process->result_data[6];
    output_refinement->class_refinement_results[current_class]
        .particle_refinement_results[current_particle]
        .yshift = result_to_process->result_data[7];
    output_refinement->class_refinement_results[current_class]
        .particle_refinement_results[current_particle]
        .defocus1 = result_to_process->result_data[8];
    output_refinement->class_refinement_results[current_class]
        .particle_refinement_results[current_particle]
        .defocus2 = result_to_process->result_data[9];
    output_refinement->class_refinement_results[current_class]
        .particle_refinement_results[current_particle]
        .defocus_angle = result_to_process->result_data[10];
    output_refinement->class_refinement_results[current_class]
        .particle_refinement_results[current_particle]
        .phase_shift = result_to_process->result_data[11];
    output_refinement->class_refinement_results[current_class]
        .particle_refinement_results[current_particle]
        .occupancy = result_to_process->result_data[12];
    output_refinement->class_refinement_results[current_class]
        .particle_refinement_results[current_particle]
        .logp = result_to_process->result_data[13];
    output_refinement->class_refinement_results[current_class]
        .particle_refinement_results[current_particle]
        .sigma = result_to_process->result_data[14];
    output_refinement->class_refinement_results[current_class]
        .particle_refinement_results[current_particle]
        .score = result_to_process->result_data[15];
    output_refinement->class_refinement_results[current_class]
        .particle_refinement_results[current_particle]
        .pixel_size = result_to_process->result_data[17];
    output_refinement->class_refinement_results[current_class]
        .particle_refinement_results[current_particle]
        .microscope_voltage_kv = result_to_process->result_data[18];
    output_refinement->class_refinement_results[current_class]
        .particle_refinement_results[current_particle]
        .microscope_spherical_aberration_mm =
        result_to_process->result_data[19];
    output_refinement->class_refinement_results[current_class]
        .particle_refinement_results[current_particle]
        .beam_tilt_x = result_to_process->result_data[20];
    output_refinement->class_refinement_results[current_class]
        .particle_refinement_results[current_particle]
        .beam_tilt_y = result_to_process->result_data[21];
    output_refinement->class_refinement_results[current_class]
        .particle_refinement_results[current_particle]
        .image_shift_x = result_to_process->result_data[22];
    output_refinement->class_refinement_results[current_class]
        .particle_refinement_results[current_particle]
        .image_shift_y = result_to_process->result_data[23];
    output_refinement->class_refinement_results[current_class]
        .particle_refinement_results[current_particle]
        .amplitude_contrast = result_to_process->result_data[24];
    output_refinement->class_refinement_results[current_class]
        .particle_refinement_results[current_particle]
        .assigned_subset = result_to_process->result_data[25];
    ...
    if (number_of_received_particle_results == 1) {
      ...
      my_parent->ShowRefinementResultsPanel->AngularPlotPanel
          ->SetSymmetryAndNumber(
              active_refinement_package->symmetry,
              long(float(output_refinement->number_of_particles) *
                   percent_used_used_multiplier));
      my_parent->Layout();
    } else if (current_time != time_of_last_update) {
      int current_percentage;
      if (number_of_rounds_run == 0)
        current_percentage = float(number_of_received_particle_results) /
                             float(output_refinement->number_of_particles) *
                             100.0;  // always 1 class for first round
      else
        current_percentage = float(number_of_received_particle_results) /
                             float(output_refinement->number_of_particles *
                                   output_refinement->number_of_classes) *
      ...
      float seconds_per_job =
          float(job_time) / float(number_of_received_particle_results - 1);

      long seconds_remaining;
      if (number_of_rounds_run == 0)
        seconds_remaining = float((input_refinement->number_of_particles) -
                                  number_of_received_particle_results) *
                            seconds_per_job;
      else
        seconds_remaining = float((input_refinement->number_of_particles *
      ...
      my_parent->TimeRemainingText->SetLabel(
          time_remaining.Format("Time Remaining : %Hh:%Mm:%Ss"));
    }

    // Add this result to the list of results to be plotted onto the angular
    // plot
    if (current_class == 0 &&
        output_refinement->class_refinement_results[current_class]
                .particle_refinement_results[current_particle]
      ...
      my_parent->ShowRefinementResultsPanel->AngularPlotPanel
          ->AddRefinementResult(
              &output_refinement->class_refinement_results[current_class]
                   .particle_refinement_results[current_particle]);
      // Plot this new result onto the angular plot immediately if it's one of
      // the first few results to come in. Otherwise, only plot at regular
      // intervals.

      if (my_parent->ShowRefinementResultsPanel->AngularPlotPanel
                      ->refinement_results_to_plot.Count() *
                  my_parent->ShowRefinementResultsPanel->AngularPlotPanel
                      ->symmetry_matrices.number_of_matrices <
              1500 ||
          current_time - my_parent->time_of_last_result_update > 0) {
    ...
    if (number_of_received_particle_results == 1) {
      ...
      time_of_last_update = current_time;
      int current_percentage = float(number_of_received_particle_results) /
                               float(expected_number_of_results) * 100.0;
      if (current_percentage > 100) current_percentage = 100;
      my_parent->ProgressBar->SetValue(current_percentage);
      long job_time = current_time - current_job_starttime;
      float seconds_per_job =
          float(job_time) / float(number_of_received_particle_results - 1);
      long seconds_remaining = float(expected_number_of_results -
                                     number_of_received_particle_results) *
                               seconds_per_job;
    ...
    output_refinement->class_refinement_results[class_number - 1]
        .class_resolution_statistics.Init(
            output_refinement->resolution_statistics_pixel_size,
            output_refinement->resolution_statistics_box_size);

    output_refinement->class_refinement_results[class_number - 1]
        .class_resolution_statistics.FSC.ClearData();
    output_refinement->class_refinement_results[class_number - 1]
        .class_resolution_statistics.part_FSC.ClearData();
    output_refinement->class_refinement_results[class_number - 1]
        .class_resolution_statistics.part_SSNR.ClearData();
    output_refinement->class_refinement_results[class_number - 1]
        .class_resolution_statistics.rec_SSNR.ClearData();
    ...
    for (int counter = 0; counter < number_of_points; counter++) {
      ...
      output_refinement->class_refinement_results[class_number - 1]
          .class_resolution_statistics.FSC.AddPoint(current_resolution, fsc);
      output_refinement->class_refinement_results[class_number - 1]
          .class_resolution_statistics.part_FSC.AddPoint(current_resolution,
                                                         part_fsc);
      output_refinement->class_refinement_results[class_number - 1]
          .class_resolution_statistics.part_SSNR.AddPoint(current_resolution,
                                                          part_ssnr);
      output_refinement->class_refinement_results[class_number - 1]
          .class_resolution_statistics.rec_SSNR.AddPoint(current_resolution,
                                                         rec_ssnr);
    }
  } else if (running_job_type == LOCAL_FILTERING) {
    // WTW chance that times are messed up now? global sharing
    ...
    if (recieved_local_filtering_job_results == 1) {
      ...
      time_of_last_update = current_time;
      int current_percentage = float(recieved_local_filtering_job_results) /
                               float(num_processes_for_local_filtering) *
                               100.0;  // get num processes
      if (current_percentage > 100) current_percentage = 100;
      my_parent->ProgressBar->SetValue(current_percentage);
      long job_time = current_time - current_job_starttime;
      float seconds_per_job =
          float(job_time) / float(recieved_local_filtering_job_results - 1);
      long seconds_remaining = float(num_processes_for_local_filtering -
                                     recieved_local_filtering_job_results) *
                               seconds_per_job;
...
void AutoRefinementManager::ProcessAllJobsFinished() {
  ...
  if (running_job_type == REFINEMENT) {
    ...
    // if this is the first round of a multiple class refinement then do random
    // occupancies. This is based on just hacking
    // input_refinement->number_of_classes = 1 in begin refinement cycle
    ...
    if (number_of_rounds_run == 0 && output_refinement->number_of_classes > 1) {
      ...
      for (class_counter = 0;
           class_counter < output_refinement->number_of_classes;
           class_counter++) {
        for (particle_counter = 0;
             particle_counter < output_refinement->number_of_particles;
             particle_counter++) {
          output_refinement->class_refinement_results[class_counter]
              .particle_refinement_results[particle_counter]
              .occupancy =
              fabsf(global_random_number_generator.GetUniformRandom() *
                    (200.0f / float(output_refinement->number_of_classes)));

          output_refinement->class_refinement_results[class_counter]
              .particle_refinement_results[particle_counter]
              .position_in_stack =
              output_refinement->class_refinement_results[0]
                  .particle_refinement_results[particle_counter]
                  .position_in_stack;
          output_refinement->class_refinement_results[class_counter]
              .particle_refinement_results[particle_counter]
              .phi = output_refinement->class_refinement_results[0]
                         .particle_refinement_results[particle_counter]
                         .phi;
          output_refinement->class_refinement_results[class_counter]
              .particle_refinement_results[particle_counter]
              .theta = output_refinement->class_refinement_results[0]
                           .particle_refinement_results[particle_counter]
                           .theta;
          output_refinement->class_refinement_results[class_counter]
              .particle_refinement_results[particle_counter]
              .psi = output_refinement->class_refinement_results[0]
                         .particle_refinement_results[particle_counter]
                         .psi;
          output_refinement->class_refinement_results[class_counter]
              .particle_refinement_results[particle_counter]
              .xshift = output_refinement->class_refinement_results[0]
                            .particle_refinement_results[particle_counter]
                            .xshift;
          output_refinement->class_refinement_results[class_counter]
              .particle_refinement_results[particle_counter]
              .yshift = output_refinement->class_refinement_results[0]
                            .particle_refinement_results[particle_counter]
                            .yshift;
          output_refinement->class_refinement_results[class_counter]
              .particle_refinement_results[particle_counter]
              .score = output_refinement->class_refinement_results[0]
                           .particle_refinement_results[particle_counter]
                           .score;
          output_refinement->class_refinement_results[class_counter]
              .particle_refinement_results[particle_counter]
          ...
          output_refinement->class_refinement_results[class_counter]
              .particle_refinement_results[particle_counter]
              .sigma = output_refinement->class_refinement_results[0]
                           .particle_refinement_results[particle_counter]
                           .sigma;
          output_refinement->class_refinement_results[class_counter]
              .particle_refinement_results[particle_counter]
              .defocus1 = output_refinement->class_refinement_results[0]
                              .particle_refinement_results[particle_counter]
                              .defocus1;
          output_refinement->class_refinement_results[class_counter]
              .particle_refinement_results[particle_counter]
              .defocus2 = output_refinement->class_refinement_results[0]
                              .particle_refinement_results[particle_counter]
                              .defocus2;
          output_refinement->class_refinement_results[class_counter]
              .particle_refinement_results[particle_counter]
          ...
          output_refinement->class_refinement_results[class_counter]
              .particle_refinement_results[particle_counter]
              .phase_shift = output_refinement->class_refinement_results[0]
                                 .particle_refinement_results[particle_counter]
                                 .phase_shift;
          output_refinement->class_refinement_results[class_counter]
              .particle_refinement_results[particle_counter]
              .logp = output_refinement->class_refinement_results[0]
                          .particle_refinement_results[particle_counter]
                          .logp;
          output_refinement->class_refinement_results[class_counter]
              .particle_refinement_results[particle_counter]
          ...
          output_refinement->class_refinement_results[class_counter]
              .particle_refinement_results[particle_counter]
              .pixel_size = output_refinement->class_refinement_results[0]
                                .particle_refinement_results[particle_counter]
                                .pixel_size;
          output_refinement->class_refinement_results[class_counter]
              .particle_refinement_results[particle_counter]
          ...
          output_refinement->class_refinement_results[class_counter]
              .particle_refinement_results[particle_counter]
              .beam_tilt_x = output_refinement->class_refinement_results[0]
                                 .particle_refinement_results[particle_counter]
                                 .beam_tilt_x;
          output_refinement->class_refinement_results[class_counter]
              .particle_refinement_results[particle_counter]
              .beam_tilt_y = output_refinement->class_refinement_results[0]
                                 .particle_refinement_results[particle_counter]
                                 .beam_tilt_y;
          output_refinement->class_refinement_results[class_counter]
              .particle_refinement_results[particle_counter]
        ...
        output_refinement->class_refinement_results[class_counter]
            .average_occupancy = 100.0f / output_refinement->number_of_classes;
      }
    ...
    my_parent->next_thread_id++;
    OrthDrawerThread *result_thread = new OrthDrawerThread(
        my_parent, current_reference_filenames,
        wxString::Format("Iter. #%i", number_of_rounds_run + 1), 1.0f,
        active_mask_radius / input_refinement->resolution_statistics_pixel_size,
        my_parent->active_orth_thread_id);

    if (result_thread->Run() != wxTHREAD_NO_ERROR) {
      my_parent->WriteErrorText(
          "Error: Cannot start result creation thread, results not displayed");
    ...
    for (class_counter = 0;
      ...
      current_reconstruction_id =
          main_frame->current_project.database.ReturnHighestReconstructionID() +
          1;
      temp_asset.reconstruction_job_id = current_reconstruction_id;
      // last_used_reconstruction_id = current_reconstruction_id;
      ...
      main_frame->current_project.database.AddReconstructionJob(
          current_reconstruction_id, active_refinement_package->asset_id,
          output_refinement->refinement_id, "", active_inner_mask_radius,
          active_mask_radius, current_resolution_limit_rec,
          current_score_weight_conversion, false, active_auto_crop, false,
          active_should_apply_blurring, active_smoothing_factor,
          class_counter + 1, long(temp_asset.asset_id));

      temp_asset.asset_name = wxString::Format(
          "Auto #%li (Rnd. %i) - Class #%i", current_output_refinement_id,
          number_of_rounds_run + 1, class_counter + 1);
      ...
      // my_parent->WriteBlueText(wxString::Format(wxT("WTW DEBUG Saved
      // Filename:%s:HM1:%s:HM2:%s:\n"), (temp_asset.filename).GetFullPath(),
      // (temp_asset.half_map_1_filename).GetFullPath(),
      // (temp_asset.half_map_2_filename).GetFullPath()));

      output_refinement->reference_volume_ids.Add(
          current_reference_asset_ids[class_counter]);
      current_reference_asset_ids[class_counter] = temp_asset.asset_id;

      // set the output volume
      output_refinement->class_refinement_results[class_counter]
          .reconstructed_volume_asset_id = temp_asset.asset_id;
      output_refinement->class_refinement_results[class_counter]
          .reconstruction_id = current_reconstruction_id;

      active_refinement_package->references_for_next_refinement.Add(
          temp_asset.asset_id);
      main_frame->current_project.database.ExecuteSQL(
          wxString::Format("UPDATE REFINEMENT_PACKAGE_CURRENT_REFERENCES_%li "
                           "SET VOLUME_ASSET_ID=%i WHERE CLASS_NUMBER=%i",
                           current_refinement_package_asset_id,
                           temp_asset.asset_id, class_counter + 1));

      volume_asset_panel->AddAsset(&temp_asset);
      main_frame->current_project.database.AddNextVolumeAsset(
          temp_asset.asset_id, temp_asset.asset_name,
          temp_asset.filename.GetFullPath(), temp_asset.reconstruction_job_id,
          temp_asset.pixel_size, temp_asset.x_size, temp_asset.y_size,
          temp_asset.z_size, temp_asset.half_map_1_filename.GetFullPath(),
          temp_asset.half_map_2_filename.GetFullPath());
    }
    ...
    if (output_refinement->number_of_classes > 1) {
      for (class_counter = 0;
           class_counter < output_refinement->number_of_classes;
           class_counter++) {
        my_parent->WriteInfoText(wxString::Format(
            wxT("Est. Res. Class %2i = %2.2f Å (%2.2f %%)"), class_counter + 1,
            output_refinement->class_refinement_results[class_counter]
                .class_resolution_statistics.ReturnEstimatedResolution(),
            average_occupancies[class_counter]));
      }
    } else {
      my_parent->WriteInfoText(wxString::Format(
          wxT("Est. Res. = %2.2f Å"),
    ...
    ArrayofAngularDistributionHistograms all_histograms =
        output_refinement->ReturnAngularDistributions(
            active_refinement_package->symmetry);

    for (class_counter = 1;
         class_counter <= output_refinement->number_of_classes;
         class_counter++) {
      main_frame->current_project.database.AddRefinementAngularDistribution(
          all_histograms[class_counter - 1], output_refinement->refinement_id,
          class_counter);
    }
    ...
    active_refinement_package->last_refinment_id =
        output_refinement->refinement_id;
    active_refinement_package->refinement_ids.Add(
        output_refinement->refinement_id);

    main_frame->current_project.database.ExecuteSQL(wxString::Format(
        "UPDATE REFINEMENT_PACKAGE_ASSETS SET LAST_REFINEMENT_ID=%li WHERE "
        "REFINEMENT_PACKAGE_ASSET_ID=%li",
        output_refinement->refinement_id, current_refinement_package_asset_id));
    main_frame->current_project.database.ExecuteSQL(wxString::Format(
        "INSERT INTO REFINEMENT_PACKAGE_REFINEMENTS_LIST_%li "
        "(REFINEMENT_NUMBER, REFINEMENT_ID) VALUES (%li, %li);",
        current_refinement_package_asset_id,
        main_frame->current_project.database.ReturnSingleLongFromSelectCommand(
            wxString::Format("SELECT MAX(REFINEMENT_NUMBER) FROM "
                             "REFINEMENT_PACKAGE_REFINEMENTS_LIST_%li",
                             current_refinement_package_asset_id)) +
            1,
        output_refinement->refinement_id));
    ...
    my_parent->ShowRefinementResultsPanel->FSCResultsPanel->AddRefinement(
        output_refinement);

    if (my_parent->ShowRefinementResultsPanel->TopBottomSplitter->IsSplit() ==
        false) {
      my_parent->ShowRefinementResultsPanel->TopBottomSplitter
          ->SplitHorizontally(
              my_parent->ShowRefinementResultsPanel->TopPanel,
              my_parent->ShowRefinementResultsPanel->BottomPanel);
...
void AutoRefinementManager::DoMasking() {
  MyDebugAssertTrue(active_should_auto_mask || active_should_mask,
                    "DoMasking called, when masking not ticked!");
  MyDebugAssertFalse(
      active_should_auto_mask && active_should_mask,
      "Masking should either be from user file, or auto, not both");
  ...
  for (int class_counter = 0;
       class_counter < current_reference_filenames.GetCount();
       class_counter++) {
    current_ref_filename = current_reference_filenames.Item(class_counter);
    current_masked_filename = main_frame->ReturnAutoRefine3DScratchDirectory() +
                              current_ref_filename.GetName();
  ...
  if (active_should_mask) {
    my_parent->WriteInfoText(
        "Masking reference reconstruction with selected mask");
    ...
    Multiply3DMaskerThread *mask_thread = new Multiply3DMaskerThread(
        ...
        wanted_low_pass_filter_radius,
        input_refinement->resolution_statistics_pixel_size,
        my_parent->active_mask_thread_id);

    if (mask_thread->Run() != wxTHREAD_NO_ERROR) {
      my_parent->WriteErrorText(
          "Error: Cannot start masking thread, masking will not be performed");
      delete mask_thread;
    } else {
    ...
    float current_res =
        input_refinement->class_refinement_results[0]
            .class_resolution_statistics.ReturnEstimatedResolution(true);
    if (current_res > class_high_res_limits[0])
      current_res = class_high_res_limits[0];
    wxPrintf("Estimated resolution = %.2f\n", current_res);
    AutoMaskerThread *mask_thread = new AutoMaskerThread(
        my_parent, current_reference_filenames, masked_filenames,
        input_refinement->resolution_statistics_pixel_size, active_mask_radius,
        my_parent->active_mask_thread_id, current_res);
    ...
    if (mask_thread->Run() != wxTHREAD_NO_ERROR) {
      ...
      return;  // just return, we will startup again whent he mask thread
               // finishes.
    }
  }
}
...
void AutoRefinementManager::CycleRefinement() {
  ...
  wxPrintf("THE OUTPUT REFINEMENT ID IS CYCLE:%li:\n",
           output_refinement->refinement_id);

  int number_of_bleed_shells =
      ceil(output_refinement->resolution_statistics_box_size /
           (active_mask_radius /
            output_refinement->resolution_statistics_pixel_size));

  // loop over all particles, to see if they were actually active, and if so
  // whether they were global or not this number should be consistent for all
  // classes, so only check class 1

  for (particle_counter = 0;
       particle_counter < output_refinement->number_of_particles;
       particle_counter++) {
    if (input_refinement->class_refinement_results[0]
                .particle_refinement_results[particle_counter]
                .image_is_active == 0 &&
        output_refinement->class_refinement_results[0]
                .particle_refinement_results[particle_counter]
      ...
      resolution_of_last_global_alignment[particle_counter] =
          high_res_limit_per_round[high_res_limit_per_round.GetCount() - 1];
    } else {
      rounds_since_global_alignment[particle_counter]++;
    }
  }

  for (class_counter = 0; class_counter < output_refinement->number_of_classes;
       class_counter++) {
    current_0p5_resolution =
        output_refinement->class_refinement_results[class_counter]
            .class_resolution_statistics.Return0p5Resolution(false);
    current_part_0p5_resolution =
        output_refinement->class_refinement_results[class_counter]
            .class_resolution_statistics.Return0p5Resolution(true);

    current_0p5_res_minus_bleed =
        output_refinement->class_refinement_results[class_counter]
            .class_resolution_statistics.ReturnResolutionNShellsBefore(
                current_0p5_resolution, number_of_bleed_shells + 1);
    current_part_0p5_res_minus_bleed =
        output_refinement->class_refinement_results[class_counter]
            .class_resolution_statistics.ReturnResolutionNShellsBefore(
                current_part_0p5_resolution, number_of_bleed_shells + 1);
    ...
    average_0p5_res_minus_bleed =
        (current_0p5_res_minus_bleed + current_part_0p5_res_minus_bleed) * 0.5f;

    bleed_resolution =
        output_refinement->class_refinement_results[class_counter]
            .class_resolution_statistics.ReturnResolutionNShellsAfter(
                class_high_res_limits[class_counter], number_of_bleed_shells);
    resolution_min_shells_after =
        output_refinement->class_refinement_results[class_counter]
            .class_resolution_statistics.ReturnResolutionNShellsAfter(
                class_high_res_limits[class_counter],
                output_refinement->resolution_statistics_box_size / 15);
    ...
    res_for_next_round =
        std::max(resolution_min_shells_after, average_0p5_res_minus_bleed);
    ...
    //current_0p5_resolution); 	wxPrintf("current_0p5_resolution_minus_bleed =
    //%.2f\n", current_0p5_res_minus_bleed); 	wxPrintf("bleed_resolution =
    //%.2f\n", bleed_resolution); 	wxPrintf("resolution_min_shells_after =
    //%.2f\n", resolution_min_shells_after); 	wxPrintf("safe_resolution =
    //%.2f\n", safe_resolution); 	wxPrintf("res_for_next_round = %.2f\n",
    //res_for_next_round); 	wxPrintf("number_bleed_shells = %i\n\n\n\n",
    //number_of_bleed_shells);

    if (output_refinement->class_refinement_results[class_counter]
            .class_resolution_statistics.Return0p5Resolution() < best_res) {
      best_res = output_refinement->class_refinement_results[class_counter]
                     .class_resolution_statistics.Return0p5Resolution();
    }
  ...
  if (resolution_per_round.GetCount() > 0) {
    if (best_p143_res >
        resolution_per_round[resolution_per_round.GetCount() - 1] -
            0.1)  // the resolution did not improve, lets add more particles to
                  // the max
    {
    ...
    if (best_p143_res < 7.0f)
      max_percent_used =
          100.0f;  // if the resolution is good, just use all the particles..
  }
  ...
  float estimated_required_asym_units =
      8000.0f * expf(75.0f / powf(best_p143_res, 2));
  long wanted_number_of_asym_units = myroundint(estimated_required_asym_units) *
                                     output_refinement->number_of_classes;
  long number_of_asym_units =
      output_refinement->number_of_particles *
      ReturnNumberofAsymmetricUnits(active_refinement_package->symmetry);
  ...
  current_percent_used =
      (float(wanted_number_of_asym_units) / float(number_of_asym_units)) *
      100.0;
  ...
  if (output_refinement->number_of_classes == 1)
    ...
    change_in_occupancies =
        output_refinement->ReturnChangeInAverageOccupancy(*input_refinement);
  }
  ...
  if (resolution_per_round.GetCount() >= min_rounds_to_run &&
      max_percent_used > 99.0 && change_in_occupancies < 1.0) {
    should_stop = true;
    float round_resolution =
        resolution_per_round[resolution_per_round.GetCount() - 3];

    for (int round_counter = resolution_per_round.GetCount() - 2;
         round_counter <= resolution_per_round.GetCount() - 1;
         round_counter++) {
  ...
  if (this_is_the_final_round == true) {
    ...
    my_parent->TimeRemainingText->SetLabel(wxString::Format(
        "All Done! (%s)", wxTimeSpan::Milliseconds(my_parent->stopwatch.Time())
                              .Format(wxT("%Hh:%Mm:%Ss"))));
...
void AutoRefine3DPanel::OnGenerateMaskThreadComplete(wxThreadEvent &my_event) {
  if (my_event.GetInt() == active_mask_thread_id)
    my_refinement_manager.OnGenerateMaskThreadComplete(my_event.GetString());
}

void AutoRefinementManager::OnGenerateMaskThreadComplete(
    wxString first_last_slice_with_data) {
  ...
  if (tokenizer.HasMoreTokens()) {
    ...
    if (tokenizer.HasMoreTokens()) {
      ...
      // maybe should error out TODO WTW
    }
  } else {
...
void AutoRefine3DPanel::OnOrthThreadComplete(
    ReturnProcessedImageEvent &my_event) {
  Image *new_image = my_event.GetImage();

  if (my_event.GetInt() == active_orth_thread_id) {
    if (new_image != NULL) {
      ShowRefinementResultsPanel->ShowOrthDisplayPanel->OpenImage(
          new_image, my_event.GetString(), true);

      if (ShowRefinementResultsPanel->LeftRightSplitter->IsSplit() == false) {
        ShowRefinementResultsPanel->LeftRightSplitter->SplitVertically(
            ShowRefinementResultsPanel->LeftPanel,
            ShowRefinementResultsPanel->RightPanel, 600);
        Layout();
      }
    }
  } else {
