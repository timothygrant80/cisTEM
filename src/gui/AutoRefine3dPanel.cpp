...
//	input_params_combo_is_dirty = false;
	...
	#ifdef EXPERIMENTAL
	Bind(wxEVT_DENMODTHREAD_COMPLETED, & AutoRefine3DPanel::OnDenmodThreadComplete, this);
	#endif
...
#ifdef EXPERIMENTAL
void AutoRefinementManager::DoDensityModification()
{
...
#ifdef EXPERIMENTAL
void AutoRefine3DPanel::OnDenmodThreadComplete (wxThreadEvent& my_event)
{
	my_refinement_manager.OnDenmodThreadComplete();
}

void AutoRefinementManager::OnDenmodThreadComplete()
{
	for (int class_counter = 0; class_counter < active_refinement_package->number_of_classes; class_counter++)
	{
		DenmodJobWrapup wrapup;
		wrapup.Init(main_frame->ReturnAutoRefine3DScratchDirectory() + wxString::Format("denmod_%li_%i/denmod.out", main_frame->current_project.database.ReturnHighestRefinementID() + 1, class_counter + 1), main_frame->current_project.volume_asset_directory.GetFullPath() + wxString::Format("/volume_%li_%i.mrc", main_frame->current_project.database.ReturnHighestRefinementID() + 1, class_counter + 1), main_frame->ReturnAutoRefine3DScratchDirectory() + wxString::Format("denmod_%li_%i/volume_%li_%i.mrc", main_frame->current_project.database.ReturnHighestRefinementID() + 1, class_counter + 1, main_frame->current_project.database.ReturnHighestRefinementID() + 1, class_counter + 1));
		wrapup.Run();
