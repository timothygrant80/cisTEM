...
void MatchTemplateResultsPanel::FillBasedOnSelectCommand(wxString wanted_command)
{
	...
	if (template_match_job_ids != NULL) delete [] template_match_job_ids;
...
void MatchTemplateResultsPanel::FillResultsPanelAndDetails(int row, int column)
{
	...
	if (current_result.job_type == TEMPLATE_MATCH_FULL_SEARCH) JobTitleStaticText->SetLabel(current_result.job_name);
	else JobTitleStaticText->SetLabel(wxString::Format("%s, Refinement of job #%li", current_result.job_name, current_result.input_job_id));
	...
	if (current_result.exclude_above_xy_threshold == true) IgnoreShiftedPeaksStaticText->SetLabel("Yes");
	else IgnoreShiftedPeaksStaticText->SetLabel("No");
