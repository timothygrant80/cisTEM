...
void MyRefinementResultsPanel::FillInputParametersComboBox(void)
{
	if (RefinementPackageComboBox->GetSelection() >= 0)
	{
		...
		if (OrthPanel->my_notebook->GetSelection() >= 0) FillAngles(OrthPanel->my_notebook->GetSelection());
		if (OrthPanel->my_notebook->GetSelection() >= 0) WriteJobInfo(OrthPanel->my_notebook->GetSelection());
...
void MyRefinementResultsPanel::UpdateBufferedFullRefinement()
{
	...
	{
		if (buffered_full_refinement != NULL) delete buffered_full_refinement;
		...
		if (OrthPanel->my_notebook->GetPageCount() != currently_displayed_refinement->number_of_classes)
		{
			...
			for (int class_counter = 0; class_counter< currently_displayed_refinement->number_of_classes; class_counter++)
			{
				...
				if (array_position != -1)
				{
					...
					{
						if (currently_displayed_refinement->number_of_classes == 1)	OrthPanel->OpenFile(current_orth_filename, wxString::Format(wxT("#%i (%.2fÅ)"), class_counter + 1, currently_displayed_refinement->class_refinement_results[0].estimated_resolution));
						else
						{
			...
			for (int class_counter = 0; class_counter< currently_displayed_refinement->number_of_classes; class_counter++)
			{
				...
				if (array_position != -1)
				{
					...
					if (DoesFileExist(current_orth_filename) == true)
					{
						...
						if (currently_displayed_refinement->number_of_classes == 1)	OrthPanel->ChangeFile(current_orth_filename, wxString::Format(wxT("#%i (%.2fÅ)"), class_counter + 1, currently_displayed_refinement->class_refinement_results[0].estimated_resolution));
						else
						{
...
void MyRefinementResultsPanel::WriteJobInfo(int wanted_class)
{
	...
	if (currently_displayed_refinement->class_refinement_results[wanted_class].should_focus_classify == true) ShouldFocusClassifyStaticText->SetLabel("Yes");
	else ShouldFocusClassifyStaticText->SetLabel("No");
	...
	if (currently_displayed_refinement->class_refinement_results[wanted_class].should_refine_ctf == true) ShouldRefineCTFStaticText->SetLabel("Yes");
	else ShouldRefineCTFStaticText->SetLabel("No");
	...
	if (currently_displayed_refinement->class_refinement_results[wanted_class].should_auto_mask == true) ShouldAutoMaskStaticText->SetLabel("Yes");
	else ShouldAutoMaskStaticText->SetLabel("No");

	if (currently_displayed_refinement->class_refinement_results[wanted_class].should_refine_input_params == true) RefineInputParamsStaticText->SetLabel("Yes");
	else RefineInputParamsStaticText->SetLabel("No");

	if (currently_displayed_refinement->class_refinement_results[wanted_class].should_use_supplied_mask == true) UseSuppliedMaskStaticText->SetLabel("Yes");
	else UseSuppliedMaskStaticText->SetLabel("No");
	...
	if (currently_displayed_refinement->class_refinement_results[wanted_class].should_low_pass_filter_mask == true) ShouldFilterOutsideMaskStaticText->SetLabel("Yes");
	else ShouldFilterOutsideMaskStaticText->SetLabel("No");
	...
	if (should_adjust_score == true) AdjustScoresStaticText->SetLabel("Yes");
	else AdjustScoresStaticText->SetLabel("No");

	if (should_crop_images == true) ShouldCropImagesStaticText->SetLabel("Yes");
	else ShouldCropImagesStaticText->SetLabel("No");

	if (should_likelihood_blur == true) ShouldLikelihoodBlurStaticText->SetLabel("Yes");
	else ShouldLikelihoodBlurStaticText->SetLabel("No");
