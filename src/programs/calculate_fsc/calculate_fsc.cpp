...
void CalculateFSC::DoInteractiveUserInput()
{
	...
	wxString res_statistics				= my_input->GetFilenameFromUser("Input data statistics", "The table listing FSC, Part_FSC, Part_SSNR and Rec_SSNR", "my_statistics.txt", false);
	float pixel_size					= my_input->GetFloatFromUser("Pixel size (A)", "Pixel size of the map in Angstroms", "1.0", 0.000001);
	float mask_radius					= my_input->GetFloatFromUser("Mask radius (A)", "Radius of mask to be applied to the input reconstructions, in Angstroms", "100.0", 0.0);
	...
	my_current_job.ManualSetArguments("ttttfffb", output_reconstruction_1.ToUTF8().data(), output_reconstruction_2.ToUTF8().data(), input_mask.ToUTF8().data(), res_statistics.ToUTF8().data(),
			pixel_size, mask_radius, molecular_mass_in_kDa, use_mask);
}
...
bool CalculateFSC::DoCalculation()
{
	...
	float molecular_mass_in_kDa			= my_current_job.arguments[6].ReturnFloatArgument();
	bool use_mask						= my_current_job.arguments[7].ReturnBoolArgument();
	...
	{
		mask_volume_in_voxels = density_map_1.CosineMask(mask_radius / pixel_size, cosine_edge / pixel_size);
		mask_volume_in_voxels = density_map_2.CosineMask(mask_radius / pixel_size, cosine_edge / pixel_size);
	}
