...
bool NikoTestApp::DoCalculation( ) {
    ...
    // wxString    outputpath     = "/groups/lingli/Documents/cisTEM/build/test_Intel-gpu-debug-static/src/Output/";
    // std::string outputpathstd  = "/groups/lingli/Documents/cisTEM/build/test_Intel-gpu-debug-static/src/Output/";
    wxString    outputpath    = "/groups/lingli/Documents/cisTEM/build/test_Intel-gpu-debug-static/src/Output/";
    std::string outputpathstd = "/groups/lingli/Documents/cisTEM/build/test_Intel-gpu-debug-static/src/Output/";
    // wxPrintf("%s\n", weightname + "zzz");
    ...
    NumericTextFile *
            input_coos_file,
            *tilt_angle_file, *peak_points, *shift_file;
    ...
    int   number_of_tilt = tilt_angle_file->number_of_lines;
    Image current_image, ref_image;
    Image weightingmap_image;

    float tilts[number_of_tilt];
    float shifts[number_of_tilt][2];
    float peaks[number_of_tilt][2];
    float phi;
    //loading the tilts into an array
    for ( int i = 0; i < number_of_tilt; i++ ) {
    ...
    // MRCFile peakfile("peaks.mrc", true);
    // peakfile.OpenFile("peaks.mrc", true);
    ...
    int   center_index;
    float sigma   = 0.05; // sigma regarding gaussian distribution. at 3 sigma, the probability down to 0.00135
    float sigma_h = 0.03;
    center_index  = int(image_no / 2);
    // current_image.WriteSlice(&peakfile, current_index);

    wxPrintf("image number in the stack: %i\n", image_no);
    float* tmpang = new float[image_no];
    // ref_image.ReadSlice(&input_stack, center_index);

    // to generating some testing images
    ref_image.ReadSlice(&input_stack, 4);
    ref_image.WriteSlicesAndFillHeader("ImageStrechScaleTest/originalimg.mrc", 1);
    ref_image.ForwardFFT( );
    ref_image.GaussianLowPassFilter(sigma);
    ref_image.BackwardFFT( );
    ref_image.WriteSlicesAndFillHeader("ImageStrechScaleTest/filteredimg.mrc", 1);

    int image_dim_x = ref_image.logical_x_dimension;
    int image_dim_y = ref_image.logical_y_dimension;

    // test square mask
    ref_image.CosineRectangularMask(image_dim_x / 4.0, image_dim_y / 2.0, 1, image_dim_x / 4.0 - 10);
    ...
    Image tmp;
    tmp.Allocate(ref_image.logical_x_dimension * 1.5, ref_image.logical_y_dimension, 1, true);
    float meanvalue = ref_image.ReturnAverageOfRealValues( );
    ref_image.ForwardFFT(false); //
    tmp.SetToConstant(0.0); // To avoid valgrind uninitialised errors, but maybe this is a waste?
    ref_image.ClipInto(&tmp, meanvalue, false);
    tmp.BackwardFFT( );
    tmp.DivideByConstant(float(ref_image.logical_x_dimension * ref_image.logical_y_dimension)); // to scale the image to the same as the intensity of the image before stretched.
    tmp.WriteSlicesAndFillHeader("ImageStrechScaleTest/strechedimg-scaled.mrc", 1);

    // // Test image shift;
    // Image rotate_image;
    // // Angles.Init(0, 0, 0, 100.0, 0.0);
    // rotate_image.CopyFrom(&ref_image);
    // // rotate_image.Allocate(ref_image.logical_x_dimension, ref_image.logical_y_dimension, true);
    // // rotate_image.ForwardFFT( );
    // // ref_image.ForwardFFT( );
    // // ref_image.RotateFourier2D(rotate_image, Angles);
    // // rotate_image.BackwardFFT( );
    // // ref_image.Rotate2D(rotate_image, Angles, 0);
    // rotate_image.PhaseShift(100, 0, 0);
    // rotate_image.WriteSlicesAndFillHeader("shiftimg.mrc", 1);
    ...
    _Float64 scale = image_dim_x * image_dim_y;
    ...
    for ( int i = 1; i <= image_no; i++ ) {
        ...
        ref_image.ReplaceOutliersWithMean(5.0f);
        // add filter
        current_image.ForwardFFT( );
        current_image.GaussianLowPassFilter(sigma);
        // current_image.GaussianHighPassFilter(sigma_h);
        ...
        ref_image.BackwardFFT( );
        // current_image.WriteSlicesAndFillHeader(wxString::Format(outputpath + "input%02i_%.3f.mrc", current_index, sigma_h).ToStdString( ), true);
        ...
        cos_stretched_image.CosineRectangularMask(image_dim_x / 2.0 - image_dim_x / 8.0, image_dim_y / 2.0 - image_dim_y / 8.0, 1, image_dim_x / 4.0);
        ref_image.CosineRectangularMask(image_dim_x / 2.0 - image_dim_x / 8.0, image_dim_y / 2.0 - image_dim_y / 8.0, 1, image_dim_x / 4.0);
        ...
        cos_stretched_image.WriteSlicesAndFillHeader(wxString::Format(outputpath + "peak%02i_%.3f_%.3f.mrc", current_index, sigma, sigma_h).ToStdString( ), true);
    }
    ...
    shifts[arr_center_index][0] = peaks[arr_center_index][0];
    shifts[arr_center_index][1] = peaks[arr_center_index][0];
    ...
    for ( int i = 1; i <= bound; i++ ) {
        if ( arr_center_index + i < image_no ) {
            shifts[arr_center_index + i][0] = peaks[arr_center_index + i][0] + shifts[arr_center_index + i - 1][0];
            shifts[arr_center_index + i][1] = peaks[arr_center_index + i][1] + shifts[arr_center_index + i - 1][1];
