...
bool CombineViaMax::DoCalculation()
{
	...
	for ( long image_counter = 0; image_counter < my_input_file_one.ReturnNumberOfSlices(); image_counter++ )
	{
        ...
        {
        	if (use_absolute == false) my_image_one.real_values[pixel_counter] = std::max(my_image_one.real_values[pixel_counter], my_image_two.real_values[pixel_counter]);
            else
            {
            	if (fabsf(my_image_two.real_values[pixel_counter]) > fabsf(my_image_one.real_values[pixel_counter])) my_image_one.real_values[pixel_counter] = my_image_two.real_values[pixel_counter];
            }
        }
