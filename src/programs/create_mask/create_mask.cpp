...
//	my_current_job.Reset(9);
	my_current_job.ManualSetArguments("ttffff", input_volume.ToUTF8().data(), output_volume.ToUTF8().data(), pixel_size, outer_mask_radius, filter_resolution, rebin_value);
}
...
bool CreateMask::DoCalculation()
{
	...
	input_image.ConvertToAutoMask(pixel_size, outer_mask_radius, filter_resolution, rebin_value);
