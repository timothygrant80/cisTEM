...
void NikoTestApp::DoInteractiveUserInput()
{
	...
	UserInput *my_input = new UserInput("NikoTest", 1.0);
...
bool NikoTestApp::DoCalculation()
{
	...
	rotated_image.Allocate(input_3d.density_map.logical_x_dimension, input_3d.density_map.logical_y_dimension, false);
	inflated_image.Allocate(4 * input_3d.density_map.logical_x_dimension, 4 * input_3d.density_map.logical_y_dimension, true);
	final_image.Allocate(input_file.ReturnXSize() / binning_factor, input_file.ReturnYSize() / binning_factor, true);
	Kernel2D *kernel_index = new Kernel2D[projection_image.real_memory_allocated / 2];
...
//	for (current_image = 1; current_image <= 100; current_image++)
	{
		...
		projection_image.CosineMask(mask_radius / pixel_size, 20.0 / pixel_size);
		projection_image.ClipInto(&inflated_image, projection_image.ReturnAverageOfRealValuesOnEdges());
//		inflated_image.QuickAndDirtyWriteSlice("junk.mrc", 1);
//		exit(0);
		projection_image.ForwardFFT();
		projection_image.SwapRealSpaceQuadrants();
		inflated_image.ForwardFFT();
		inflated_image.SwapRealSpaceQuadrants();

		projection_image.RotateQuadrants(rotated_image, 90);
		rotated_image.SwapRealSpaceQuadrants();
		rotated_image.QuickAndDirtyWriteSlice("junkr1.mrc", 1);
//		projection_image.SwapRealSpaceQuadrants();
//		projection_image.QuickAndDirtyWriteSlice("junkr1.mrc", 1);
//		exit(0);

		rotation_angle.GenerateRotationMatrix2D(90.0);

		projection_image.RotateFourier2DIndex(kernel_index, rotation_angle);
		my_time_in = wxDateTime::UNow();
		for (i = 0; i < 1000; i++)
		{
			projection_image.RotateFourier2DFromIndex(rotated_image, kernel_index);
		}
		my_time_out = wxDateTime::UNow(); wxPrintf("indexed: ms taken = %li\n", my_time_out.Subtract(my_time_in).GetMilliseconds());
		rotated_image.SwapRealSpaceQuadrants();
		rotated_image.QuickAndDirtyWriteSlice("junkr2.mrc", 1);
		...
		for (i = 0; i < 1000; i++)
		{
			projection_image.RotateFourier2D(rotated_image, rotation_angle);
		}
		my_time_out = wxDateTime::UNow(); wxPrintf("bilinear: ms taken = %li\n", my_time_out.Subtract(my_time_in).GetMilliseconds());
		rotated_image.SwapRealSpaceQuadrants();
		rotated_image.QuickAndDirtyWriteSlice("junkr1.mrc", 1);
...
//		for (i = 0; i < 1; i++)
//		{
//			inflated_image.RotateFourier2D(rotated_image, rotation_angle, 1.0, true);
//		}
//		my_time_out = wxDateTime::UNow(); wxPrintf("nearest: ms taken = %li\n", my_time_out.Subtract(my_time_in).GetMilliseconds());

		if (apply_CTF) rotated_image.ApplyCTF(my_ctf);
		if (apply_shifts) rotated_image.PhaseShift(temp_float[4] / pixel_size, temp_float[5] / pixel_size);
		rotated_image.SwapRealSpaceQuadrants();

		rotated_image.BackwardFFT();
		rotated_image.ClipInto(&final_image);
...
//			wxPrintf("sigma_signal = %f, sigma_noise = %f\n", sqrtf(variance), sqrtf(variance / wanted_SNR));
		...
		if (apply_mask) final_image.CosineRingMask(15.0, mask_radius / pixel_size, 6.0);
