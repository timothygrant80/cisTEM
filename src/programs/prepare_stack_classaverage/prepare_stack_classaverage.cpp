...
void PrepareStackApp::DoInteractiveUserInput()
{
	...
	my_current_job.ManualSetArguments("ttttffbiiifffbbii",	input_particle_images.ToUTF8().data(),
...
bool PrepareStackApp::DoCalculation()
{
	...
	wxString input_parameter_file 				= my_current_job.arguments[2].ReturnStringArgument();
	...
	float  microscope_voltage					= my_current_job.arguments[10].ReturnFloatArgument();
	float microscope_cs 						= my_current_job.arguments[11].ReturnFloatArgument();
	float amplitude_contrast 					= my_current_job.arguments[12].ReturnFloatArgument();
	bool invert_contrast						= my_current_job.arguments[13].ReturnBoolArgument();
	bool process_a_subset						= my_current_job.arguments[14].ReturnBoolArgument();
	int  first_classaverage					    = my_current_job.arguments[15].ReturnIntegerArgument();
	int  last_classaverage					    = my_current_job.arguments[16].ReturnIntegerArgument();
	...
	for (current_classaverage = first_classaverage; current_classaverage <= last_classaverage; current_classaverage++)
	{
		...
		for (line_counter = 0; line_counter <= input_par_file.number_of_lines; line_counter++)
		{
			...
			if (int(input_parameters[7]) == classaverages_to_make[current_classaverage])
			{
		...
		for (class_counter = 0; class_counter < number_of_classes; class_counter++)
		{
			...
			variance = sum_image.ReturnVarianceOfRealValues(sum_image.physical_address_of_box_center_x - mask_falloff / (pixel_size * bin_factor), 0.0, 0.0, 0.0, true);
			average = sum_image.ReturnAverageOfRealValues(sum_image.physical_address_of_box_center_x - mask_falloff / (pixel_size * bin_factor), true);
