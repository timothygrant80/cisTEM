...
//				comparison_object->particle->alignment_parameters.ReturnShiftX(), comparison_object->particle->alignment_parameters.ReturnShiftY());
	...
	if (isnan(tmp_corr) || isnan(tmp_penalty))
	{
		...
		wxPrintf("shift x = %f\nshift y = %f\npsi   = %f\nphi   = %f\ntheta = %f\n",
				 ...
				 comparison_object->particle->alignment_parameters.ReturnThetaAngle());
		wxPrintf("shift x = %f\nshift y = %f\npsi   = %f\nphi   = %f\ntheta = %f\n",
				 comparison_object->particle->temp_parameters.x_shift,
				 comparison_object->particle->temp_parameters.y_shift,
				 comparison_object->particle->temp_parameters.psi,
				 comparison_object->particle->temp_parameters.phi,
				 comparison_object->particle->temp_parameters.theta);
	}
...
#pragma omp critical
			...
			if ((refine_particle_local.number_of_search_dimensions > 0) && (global_search_local || local_refinement_local))
			{
				...
				if (local_refinement_local)
				{
					//				my_time_in = wxDateTime::UNow();
					comparison_object.reference_volume = &input_3d_local;
					//input_3d_local.density_map->WriteSlicesAndFillHeader("/data/wtwoods/test_local_filtering/local_filtering_small_protein/input_3d_local_density_map.mrc", pixel_size);
					comparison_object.projection_image = &projection_image_local;
					//projection_image_local.WriteSlicesAndFillHeader("/data/wtwoods/test_local_filtering/local_filtering_small_protein/projection_image_local.mrc", pixel_size);
					comparison_object.particle = &refine_particle_local;
					refine_particle_local.MapParameters(cg_starting_point);
					//wxPrintf("WTW DEBUG cg_starting_point:%f:\n", cg_starting_point);
