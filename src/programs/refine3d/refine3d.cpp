...
//	filter_constant = my_input->GetFloatFromUser("Tuning parameters: filter constant", "Constant determining how strongly data with small CTF values is suppressed during particle alignment", "1.0", 1.0);
	...
	my_current_job.Reset(44);
	my_current_job.ManualSetArguments("ttttbttttiifffffffffffffifffffffffbbbbbbbbbb",	input_particle_images.ToUTF8().data(),
																						...
																						apply_2D_masking, ctf_refinement);
}
...
//	my_output_par_shifts_file.WriteCommentLine("C    Particle#            Psi          Theta            Phi         ShiftX         ShiftY            Mag     Micrograph       Defocus1       Defocus2       AstigAng      Occupancy           LogP NormSigmaNoise          Score");
	...
	else
	{
		...
		input_3d.GenerateDefaultStatistics();
	}
	...
	if (global_search)
	{
		...
		wxPrintf("\nBinning factor for search = %f, new pixel size = %f, resolution limit = %f\nAngular step size = %f, in-plane = %f\n", binning_factor_search, search_reference_3d.pixel_size, search_reference_3d.pixel_size * 2.0, angular_step, psi_step);
	}
...
//		logp = refine_particle.ReturnLogLikelihood(input_image, final_image, pixel_size, classification_resolution_limit, alpha, sigma);
		logp = refine_particle.ReturnLogLikelihood(input_3d, projection_image, classification_resolution_limit, alpha, sigma);
