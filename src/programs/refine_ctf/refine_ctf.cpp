...
void RefineCTFApp::DoInteractiveUserInput()
{
	...
	bool beamtilt_refinement = my_input->GetYesNoFromUser("Determine beamtilt", "Should the beam tilt be refined?", "No");
...
//	int cache_pointer, current_line;
	...
	output_star_file.AddCommentToHeader("# Output parameter changes:                " + ouput_shift_file);
	...
	output_star_file.AddCommentToHeader("# Refine beamtilt:                         " + BoolToYesNo(beamtilt_refinement));
...
//			input_image_local.ReadSlice(&input_stack, int(input_parameters[0] + 0.5f));
	...
	#pragma omp parallel num_threads(max_threads) default(none) shared(parameter_average, parameter_variance, input_3d, input_star_file, input_stack, phase_difference_sum, max_threads, \
		...
		binning_factor_refine, cg_accuracy, low_resolution_limit, input_statistics, output_star_file, current_image) \
	private(i, image_counter, refine_particle, current_image_local, phase_difference_sum_local, input_parameters, temp_float, output_parameters, input_ctf, variance, average, comparison_object, \
		best_score, defocus_i, score, cg_starting_point, input_image_local, phase_difference_image_local, unbinned_image, \
		binned_image, projection_image_local, best_defocus_i, current_line)
	{ // for omp
	...
	for (current_image_local = 0; current_image_local < input_star_file.ReturnNumberofLines(); current_image_local++)
	{
		#pragma omp critical
		{
			...
			current_image++;
			if (input_parameters.position_in_stack >= first_particle && input_parameters.position_in_stack <= last_particle) input_image_local.ReadSlice(&input_stack, input_parameters.position_in_stack);
		}
		if (input_parameters.position_in_stack < first_particle || input_parameters.position_in_stack > last_particle) continue;
		image_counter++;
...
//		refine_particle.SetIndexForWeightedCorrelation();
		...
		input_ctf.Init(voltage_kV, spherical_aberration_mm, amplitude_contrast, input_parameters.defocus_1, input_parameters.defocus_2, input_parameters.defocus_angle, 0.0, 0.0, 0.0, pixel_size, input_parameters.phase_shift);
...
//		refine_particle.particle_image->MultiplyByConstant(binning_factor_refine);
		...
		refine_particle.InitCTF(voltage_kV, spherical_aberration_mm, amplitude_contrast, input_parameters.defocus_1, input_parameters.defocus_2, input_parameters.defocus_angle, input_parameters.phase_shift);
		if (ctf_refinement)
		{
			...
			for (defocus_i = - myround(float(defocus_search_range)/float(defocus_step)); defocus_i <= myround(float(defocus_search_range)/float(defocus_step)); defocus_i++)
			{
				...
				refine_particle.InitCTFImage(voltage_kV, spherical_aberration_mm, amplitude_contrast, input_parameters.defocus_1 + defocus_i * defocus_step, input_parameters.defocus_2 + defocus_i * defocus_step, input_parameters.defocus_angle, input_parameters.phase_shift);
	...
	//			refine_particle.WeightBySSNR(input_3d.statistics.part_SSNR, 1);
		...
		refine_particle.InitCTFImage(voltage_kV, spherical_aberration_mm, amplitude_contrast, output_parameters.defocus_1, output_parameters.defocus_2, input_parameters.defocus_angle, input_parameters.phase_shift);
...
//		refine_particle.CosineMask(false, true, 0.0);
		...
		output_parameters.logp = refine_particle.ReturnLogLikelihood(input_image_local, unbinned_image, input_ctf, input_3d, input_statistics, 2.0f * pixel_size, &phase_difference_image_local);
