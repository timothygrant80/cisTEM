...
void RefineCTFApp::DoInteractiveUserInput()
{
	...
	my_current_job.ManualSetArguments("ttttbttiiffffffffffffbbbbbbbiiit",
...
bool RefineCTFApp::DoCalculation()
{
	...
	int		 image_number_for_gui				= my_current_job.arguments[28].ReturnIntegerArgument();
	int		 number_of_jobs_per_image_in_gui	= my_current_job.arguments[29].ReturnIntegerArgument();
	int		 max_threads						= my_current_job.arguments[30].ReturnIntegerArgument();
	wxString directory_for_results				= my_current_job.arguments[31].ReturnStringArgument();
...
//			input_image_local.ReadSlice(&input_stack, int(input_parameters[0] + 0.5f));
			...
			input_image_local.CosineMask(outer_mask_radius / pixel_size, mask_falloff / pixel_size, true);
	...
	#pragma omp parallel num_threads(max_threads) default(none) shared(parameter_average, parameter_variance, input_3d, input_par_file, input_stack, phase_difference_sum, images_to_process, max_threads, \
...
//	phase_difference_sum.PhaseImage();

	// delete 2
//	phase_difference_sum.QuickAndDirtyReadSlice("junk.mrc",1);
//	phase_difference_sum.ForwardFFT();
//	phase_difference_image.CopyFrom(&phase_difference_sum);
//	phase_difference_sum.QuickAndDirtyWriteSlice("junk.mrc",1);
//	exit(0);

	if (beamtilt_refinement)
	{
		// Find beam tilt direction
		phase_difference_sum.ComputeAmplitudeSpectrumFull2D(&phase_difference_image, true);
		// delete 1
//		phase_difference_image.QuickAndDirtyWriteSlice("phase_difference.mrc",1);
//		phase_difference_image.QuickAndDirtyReadSlice("saved.mrc",1);
//		phase_difference_image.MultiplyByConstant(deg_2_rad(1.0f));
		phase_difference_image.Binarise(0.0f);
		input_image.CopyFrom(&phase_difference_image);
		input_image.ApplyMirrorAlongY();
		...
		for (i = 0; i < 3; i++)
		{
			for (beamtilt_azimuth_i = - beamtilt_azimuth_n; beamtilt_azimuth_i < beamtilt_azimuth_n; beamtilt_azimuth_i++)
			{
				...
				input_image.Rotate2D(beamtilt_image, rotation_matrix, pixel_size / high_resolution_limit * beamtilt_image.logical_x_dimension);
				beamtilt_image.SubtractImage(&phase_difference_image);
//				beamtilt_image.CosineMask(pixel_size / high_resolution_limit * beamtilt_image.logical_x_dimension, mask_falloff / pixel_size);
				beamtilt_image.real_values[0] = 0.0f;
				score = beamtilt_image.ReturnSumOfSquares(pixel_size / high_resolution_limit * beamtilt_image.logical_x_dimension);
				if (score < best_score)
				{
					best_score = score;
...
//					wxPrintf("score, azimuth = %g %g\n", best_score, best_azimuth / 2.0f);
		...
		best_azimuth /= 2.0f;
		// delete 1
//		best_azimuth = -0.104894;
		azimuth_cos = cosf(best_azimuth);
		azimuth_sin = sinf(best_azimuth);
		...
		best_score = std::numeric_limits<float>::max();
//		phase_difference_image.Binarise(0.0f);
//		phase_difference_image.QuickAndDirtyWriteSlice("j1.mrc", 1);
...
//			wxPrintf("i = %i beamtilt_n, beamtilt_step, beamtilt_offset = %i %g %g\n", i, beamtilt_n, beamtilt_step, beamtilt_offset);
			for (beamtilt_i = - beamtilt_n; beamtilt_i < beamtilt_n; beamtilt_i++)
			{
				...
				// delete 1
//				beamtilt = 0.001f;
				input_ctf.SetBeamTilt(beamtilt * azimuth_cos, - beamtilt * azimuth_sin);
				input_image.CalculateBeamTiltImage(input_ctf);
				input_image.ComputeAmplitudeSpectrumFull2D(&beamtilt_image, true);
				// delete 1
//				input_image.CopyFrom(&beamtilt_image);
				beamtilt_image.Binarise(0.0f);
//				beamtilt_image.QuickAndDirtyWriteSlice("j2.mrc", 1);
				beamtilt_image.SubtractImage(&phase_difference_image);
//				beamtilt_image.QuickAndDirtyWriteSlice("j3.mrc", 1);
//				exit(0);
//				beamtilt_image.CosineMask(pixel_size / high_resolution_limit, 2.0f / beamtilt_image.logical_x_dimension);
//				beamtilt_image.complex_values[0] = 0.0f + I * 0.0f;
				score = beamtilt_image.ReturnSumOfSquares(pixel_size / high_resolution_limit * beamtilt_image.logical_x_dimension);
				if (score < best_score)
				{
					best_score = score;
					best_tilt = beamtilt;
					// delete 3
//					beamtilt_x = beamtilt * azimuth_cos;
//					beamtilt_y = - beamtilt * azimuth_sin;
//					wxPrintf("score, tilt x,y = %g %g %g\n", best_score, beamtilt_x, beamtilt_y);
					// delete 5
//					beamtilt_image.ComputeAmplitudeSpectrumFull2D(&temp_image, true);
//					beamtilt_image.InvertRealValues();
//					beamtilt_image.QuickAndDirtyWriteSlice("diff_image.mrc", 1);
//					input_image.ComputeAmplitudeSpectrumFull2D(&beamtilt_image, true);
//					beamtilt_image.QuickAndDirtyWriteSlice("beamtilt_image.mrc", 1);
				}
			}
			beamtilt_n = 30;
			beamtilt_step /= 30.0;
...
//	phase_difference_sum.ComputeAmplitudeSpectrumFull2D(&phase_difference_image, true);
//	phase_difference_sum.BackwardFFT();
//	temp_float = deg_2_rad(sqrtf(float(phase_difference_sum.number_of_real_space_pixels)));
//	temp_float = rad_2_deg(1.0f);
//	phase_difference_image.MultiplyByConstant(temp_float);
//	phase_difference_image.ForwardFFT();
//	phase_difference_image.CosineMask(pixel_size / 20.0f, pixel_size / mask_falloff);
//	phase_difference_image.BackwardFFT();
//	phase_difference_image.QuickAndDirtyWriteSlice("phase_difference.mrc",1);
