...
void SamplesPrintTestStartMessage(wxString message, bool bold)
{
  ...
  if (bold) SamplesPrintBold(message);
  else SamplesPrintUnderlined(message);
  wxPrintf("\n\n");
}

void SamplesPrintUnderlined(wxString message)
{
    if (OutputIsAtTerminal() == true) wxPrintf(ANSI_UNDERLINE + message + ANSI_UNDERLINE_OFF);
    else wxPrintf("%s", message);
}

void SamplesPrintBold(wxString message)
{
    if (OutputIsAtTerminal() == true) wxPrintf(ANSI_BOLD + message + ANSI_BOLD_OFF);
    else wxPrintf("%s", message);
}
