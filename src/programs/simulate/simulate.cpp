...
void SimulateApp::DoInteractiveUserInput()
{
  ...
  bool require_pdb_to_exist = true;
    ...
    if (do3d) { 
        ...
        if ( ! found_the_best_binning ) { bin3d = 1; }
...
//        number_of_images = ceil(tilt_range/tilt_inc)*2 +1;
        ...
        for (int iTilt=0; iTilt < number_of_images; iTilt++)
        {
            ...
            tilt_psi[iTilt] = tilt_axis + this->stdErr * my_rand.GetNormalRandomSTD(0.0f,in_plane_sigma);; // *(2*PI);
...
//            rotate_waters.SetToRotation(-tilt_phi[iTilt],-tilt_theta[iTilt],-tilt_psi[iTilt]);
  ...
  if (SCALE_DEFOCUS_TO_MATCH_300)
  {
    ...
    if (DO_PRINT) { wxPrintf("Scaling the mean_defocus by %6.6f to match the def at 300 KeV\n", scale_defocus); }
  }
...
//        wave_function.do_beam_tilt_full = true;
        ...
        if (make_tilt_series) tilt_to_scale_search_range = 0.0f;//fabsf(tilt_theta[iTilt]);
